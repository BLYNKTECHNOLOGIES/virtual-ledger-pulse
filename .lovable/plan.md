...
A single test email is sent to shubham.singh@blynkex.com from **hr@blynkex.com** (the configured HR mailbox), rendered with a real recent exception day so the wording, numbers and link are exactly what employees would receive. Nothing else is turned on until it is approved.
...
- If the employee disagrees: raise an **Attendance Regularization Request** from the ERP profile → Attendance tab, with a direct button to https://erp.blynkex.com/profile?tab=attendance.
- HR signature/footer, same visual style as the existing regularization emails.

## When it sends

- Runs hourly. Picks up days marked absent/half_day where the mark is **at least 24 hours old** and no notice was sent yet for that employee+date.
...
- New `hr-attendance-exception-notify` (service role, `verify_jwt` off, zod-validated body):
  ...
  - Sends via SMTP using the `hr_mailboxes` row for hr@blynkex.com (same denomailer + secret-name pattern as `hr-mail-send`), so the From address is the HR mailbox the owner already configured.
...
1. Send the sample email to shubham.singh@blynkex.com and stop for approval.
