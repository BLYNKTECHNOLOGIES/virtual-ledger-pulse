...
export function MerchantStateCard() {
  ...
  return (
    <div className={`flex h-9 shrink-0 items-center gap-2 rounded-md border px-2.5 ${hasRestriction || isCachedOnly ? 'border-warning/30 bg-warning/5' : 'border-border bg-card'}`}>
      ...
      <Button variant="ghost" size="icon" onClick={() = aria-label="Refresh"> refreshState.mutate()} disabled={refreshState.isPending || userDetail.isLoading} className="h-6 w-6 text-muted-foreground hover:text-foreground">
