...
/** The pre-existing company document library (public.documents) — behaviour unchanged. */
export function CompanyLibraryTab() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowDialog(false);
      setForm({ title: "", description: "", category: "general", file_path: "", file_type: "pdf" });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button onClick={() => setShowDialog(true)} className="h-9"><Plus className="h-4 w-4 mr-2" /> Add document</Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          ...
          <div className="col-span-full">
            <EmptyState icon={FileText} title="No documents found" description="Add your first document to the library."
              action={<Button onClick={() => setShowDialog(true)} className="h-9"><Plus className="h-4 w-4 mr-2" />Add document</Button>} />
          </div>
        ) : filtered.map((d: any) => (
          <Card key={d.id} className="hover:shadow-sm transition-shadow">
            <CardContent className="p-5">
              <div className="flex items-start gap-3">
                <div className="p-2 bg-primary/10 rounded-lg shrink-0"><FileText className="h-5 w-5 text-primary" /></div>
                ...
                <Button size="sm" variant="ghost" className="text-destructive shrink-0" onClick={() => deleteMutation.mutate(d.id)}>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle className="text-sm font-semibold flex items-center gap-2"><FileText className="h-4 w-4 text-primary" />Add document</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>Title</Label><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Document title" className="h-9 mt-1 text-foreground" /></div>
            <div><Label>Description</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="mt-1 text-foreground" /></div>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => createMutation.mutate()} disabled={!form.title} className="h-9">Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
export default CompanyLibraryTab;
