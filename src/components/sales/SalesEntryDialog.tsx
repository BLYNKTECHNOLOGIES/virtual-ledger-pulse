...
export function SalesEntryDialog({ open, onOpenChange }: SalesEntryDialogProps) {
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('products')
        .select('*')
        .gt('current_stock_quantity', 0); // Only show products with stock
  ...
  const createSalesMutation = useMutation({
    mutationFn: async (salesData: SalesEntryForm) => {
      ...
      // 4. Create client entry with HIGH risk appetite
      ...
      // Don't throw error if client already exists
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Sales order has been successfully created and inventory updated.",
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={createSalesMutation.isPending}>
              {createSalesMutation.isPending ? "Creating..." : "Submit"}
            </Button>
          </div>
        </form>
      </DialogContent>
