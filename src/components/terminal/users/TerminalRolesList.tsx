...
import { Shield, Pencil, Plus, RefreshCw } from "lucide-react";
...
export function TerminalRolesList() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    setRoles((data || []).map((r: any) => ({
      id: r.id, name: r.name, description: r.description || "", is_default: r.is_default, permissions: r.permissions || [],
    })));
    setIsLoading(false);
  }, []);
  ...
  const openNew = () => {
    setEditingRole({ id: "", name: "", description: "", is_default: false, permissions: [] });
    setIsNew(true);
    setEditName("");
    setEditDesc("");
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={fetchRoles}>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className={`text-xs ${roleBadgeClass(role.name)}`}>
                    {role.name}
                  </Badge>
                  {role.is_default && (
