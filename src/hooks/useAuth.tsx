  ...
  const buildUserFromUserId = async (userId: string, emailFallback: string): Promise<User | null> => {
    ...
    if (roles.length === 0) {
      const { data: fallbackData } = await supabase
        .from('users')
        .select(`id, username, email, first_name, last_name, phone, avatar_url, status, email_verified, last_login, failed_login_attempts, account_locked_until, created_at, updated_at, role_id, last_activity, force_logout_at, badge_id, created_by, force_password_change, department_id, position_id, roles!role_id(id, name, description)`)
        .eq('id', userId)
        .single();
