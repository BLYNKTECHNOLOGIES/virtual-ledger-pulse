...
export async function buildVerificationPack(period: string): Promise<VerificationPack> {
  ...
  const dry = async (fn: string) => {
    const { data, error } = await supabase.functions.invoke(fn, { body: { period, dry_run: true } });
    if (error) throw new Error(`${fn}: ${error.message}`);
    if ((data as any)?.error) throw new Error((data as any).message || (data as any).error);
    return data as any;
  };

  const [lopRes, coRes, additions, deductions, recoveries, fnf, deposits, employees, workInfo, depts, rzpMap, me] =
    await Promise.all([
      ...
      fetchAllPaginated<any>(() =>
        (supabase as any).from("hr_fnf_settlements").select("*").eq("payroll_month", period).order("id"),
      ),
      fetchAllPaginated<any>(() =>
        (supabase as any).from("hr_deposit_transactions").select("*").eq("period_month", period).order("id"),
      ),
      fetchAllPaginated<any>(() =>
        (supabase as any)
          .from("hr_employees")
  ...
  const metaFor = (title: string, extra: string[][] = []): string[][] => [
    ...
    ...extra,
    ["Note", "Read-only snapshot. Figures are the live calculation at the time of download."],
  ];
  ...
  const leaveHeader = [
    ...
    "Comp-off encashed (days)", "Comp-off encashment amount", "Comp-off closing",
  ...
  for (const r of lopRows) {
    ...
    leaveRows.push([
      ...
      coOpening, coEarned, coTaken, coOffset, coEncashDays, n2(cor?.amount), coClosing,
  ...
  for (const row of deposits) {
    const amt = n2(row.amount);
    lines.push({
      badge: empBadge(row.employee_id), name: empName(row.employee_id),
      dir: amt < 0 ? "Addition" : "Deduction", cat: "Deposit",
      label: row.description || `${row.deposit_type ?? "Deposit"} — ${row.transaction_type ?? ""}`.trim(),
      amt: Math.abs(amt), origin: "Automatic", prov: "Final", payable: true,
      pushed: "—", pushedAt: "", verified: "—",
      notes: row.balance_after !== null && row.balance_after !== undefined ? `Balance after: ${n2(row.balance_after)}` : "",
    });
  }
  for (const row of fnf) {
    lines.push({
      badge: empBadge(row.employee_id), name: empName(row.employee_id),
      dir: n2(row.net_payable) < 0 ? "Deduction" : "Addition", cat: "F&F settlement",
      label: `Full & final settlement (last working day ${dmy(row.last_working_day)})`,
      amt: Math.abs(n2(row.net_payable)), origin: "Automatic", prov: row.status === "approved" ? "Final" : String(row.status ?? ""),
      payable: true,
      pushed: row.razorpay_pushed_at ? "Yes" : "No", pushedAt: istStamp(row.razorpay_pushed_at),
      verified: row.razorpay_push_status ?? "—",
      notes: [
        `Pending salary ${n2(row.pending_salary)}`,
        `Leave encashment ${n2(row.leave_encashment_amount)} (${n2(row.leave_encashment_days)} d)`,
        `Bonus ${n2(row.bonus_amount)}`,
        `Loan recovery ${n2(row.loan_recovery)}`,
        `Deposit refund ${n2(row.deposit_refund)}`,
        `Penalties ${n2(row.penalty_deductions)}`,
      ].join("; "),
    });
  }
  ...
  const summaryHeader = [
    ...
    "Per-day rate", "Divisor (calendar days)", "LOP amount",
    "Monthly gross base", "Base source", "Total additions", "Total deductions", "Expected net effect vs base",
    "Flags",
  ];
  ...
  for (const r of lopRows) {
    ...
    const perDay = base ? n2(base / daysInMonth) : 0;
    ...
    summaryRows.push([
      ...
      n2(r.raw_lop_days), n2(r.compoff_offset_days), n2(r.cl_offset_days), n2(r.lop_days),
      perDay, daysInMonth, n2(r.amount),
