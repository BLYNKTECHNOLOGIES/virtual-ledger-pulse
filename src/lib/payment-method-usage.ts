...
export async function computeSalesPaymentMethodUsage(
  ...
  if (isPaymentGateway) {
    // For gateways, sum pending settlements
    ...
    return (data || []).reduce((sum, row) => sum + Number(row.settlement_amount || 0), 0);
  }

  // For non-gateways, sum sales orders since last reset
  let query = supabase
    .from("sales_orders")
    .select("total_amount")
  ...
  if (lastReset) {
    query = query.gte("created_at", lastReset);
  }
...
export async function batchComputeSalesUsage(
  ...
  // Batch fetch pending settlements for gateways
  ...
  // Batch fetch sales orders for direct methods
  if (directIds.length > 0) {
    ...
    (orders || []).forEach((o) => {
      ...
      // Only count orders since last_reset
