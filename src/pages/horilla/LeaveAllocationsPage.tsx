  ...
  return (
    <div className="space-y-6 page-mount">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Leave Allocations</h1>
          <p className="text-sm text-muted-foreground-foreground">Quarterly leave allocation — all leaves carry forward infinitely</p>
        </div>
        ...
        {[
          ...
          <Card key={s.label}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div><p className="text-2xl font-bold">{s.value}</p><p className="text-xs text-muted-foreground-foreground">{s.label}</p></div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      {isLoading ? (
        <p className="text-center text-muted-foreground-foreground py-12">Loading...</p>
      ) : groupedArr.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            <CalendarDays className="h-10 w-10 mx-auto text-muted-foreground mb-3" />
            <p className="text-muted-foreground-foreground text-sm">No leave allocations for {getQuarterLabel(quarter)} {year}</p>
                ...
                <CardContent className="p-5">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground-foreground">{g.employee?.badge_id}</p>
                    </div>
                  </div>
                    ...
                    {g.allocations.map((a: any) => {
                      ...
                      return (
                        <div key={a.id} className="bg-muted/50 rounded-lg p-3 border border-border">
                          ...
                          <div className="flex justify-between text-[10px] text-muted-foreground-foreground">
                            <span>This Qtr: {a.allocated_days}d</span>
                            <span className="font-medium text-foreground">Bal: {cumulativeAvailable}</span>
                          </div>
                          <p className="text-[10px] text-muted-foreground-foreground mt-0.5">Used: {cumBal?.totalUsed || a.used_days} (all time)</p>
                        </div>
                      );
                    })}
                  </div>
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground-foreground">Quarter: {getQuarterLabel(quarter)} {year} • All unused days carry forward automatically</p>
          </div>
          <DialogFooter>
            ...
            <div className="bg-muted/50 rounded-lg p-3 space-y-1 text-sm">
              {leaveTypes.map((lt: any) => (
                <div key={lt.id} className="flex justify-between">
                  <span className="text-muted-foreground-foreground">{lt.name}</span>
                  <span className="font-medium">{lt.max_days_per_year} days/quarter</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-muted-foreground-foreground">💡 Unused days from previous quarters automatically carry forward.</p>
