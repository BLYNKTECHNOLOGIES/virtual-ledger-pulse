    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {zones.slice(0, 6).map((zone: any) => {
          ...
          return (
            <div key={zone.id} className="bg-card rounded-xl border border-border p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{count} Candidate{count !== 1 ? "s" : ""}</p>
              </div>
            </div>
          );
        })}
          ...
          {zones.map((zone: any) => {
            ...
            return (
              <div key={zone.id} className="bg-card rounded-xl border border-border overflow-hidden">
                <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/50" onClick={() => toggleExpand(zone.id)}>
                  {isExpanded ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
                  <Target className="h-4 w-4 text-[#E8604C]" />
                  <div className="flex-1">
                    <span className="text-sm font-semibold text-foreground">{zone.zone_name}</span>
                      ...
                      <div className="divide-y divide-gray-50">
                        {candidates.map((zc: any) => (
                          <div key={zc.id} className="flex items-center gap-3 px-4 py-2 pl-12">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-foreground">{zc.hr_candidates?.name}</p>
