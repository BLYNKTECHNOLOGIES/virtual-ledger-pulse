...
function AppealRow({ c, statusMap, canChat, onOpen, onChat }: { c: TerminalAppealCase; statusMap: Map<string, AuthoritativeOrderStatus>; canChat: boolean; onOpen: () => void; onChat: () => void }) {
  ...
  return <TableRow className="cursor-pointer hover:bg-secondary/40" onClick={onOpen}>
    <TableCell>{isActive ? <Badge variant="outline" className="text-[10px] t-mono border-primary/30 text-primary bg-primary/5"><Clock className="h-3 w-3 mr-1" />{formatDuration(age)}</Badge> : <Badge variant="outline" className="text-[10px] border-muted text-muted-foreground">Closed</Badge>}</TableCell>
    <TableCell>{!isActive ? <Badge variant="outline" className="text-[10px] border-muted text-muted-foreground">Stopped</Badge> : needsTimer ? <Badge variant="outline" className="text-[10px] border-destructive/30 text-destructive bg-destructive/5">Select timer</Badge> : c.response_due_at ? <Badge variant="outline" className={`text-[10px] t-mono ${responseExpired ? 'border-destructive/30 text-destructive bg-destructive/5' : 'border-warning/30 text-warning bg-warning/5'}`}>{responseExpired ? 'Overdue' : formatDuration(Math.max(0, Math.floor((new Date(c.response_due_at).getTime() - Date.now()) / 60000)))}</Badge> : c.response_timer_set_at ? <Badge variant="outline" className="text-[10px]">No timer · {getAppealUserName(c.timerSetBy)}</Badge> : <Badge variant="outline" className="text-[10px]">No timer</Badge>}</TableCell>
    <TableCell><div className="flex flex-col gap-1"><span className="text-xs t-mono text-foreground">{c.order_number}</span><span className="text-[10px] text-muted-foreground">{c.adv_no || 'No Ad ID'}</span><div className="flex flex-wrap gap-1"><Badge variant="outline" className={`w-fit text-[9px] ${orderStyle.badgeClass}`}>{orderStyle.label}</Badge>{isActiveAppealCase(c) && <Badge variant="outline" className="w-fit text-[9px] border-primary/30 text-primary bg-primary/5">Under Appeal</Badge>}</div></div></TableCell>
    <TableCell><div className="text-xs t-mono">{Number(c.total_price || 0).toLocaleString('en-IN')} {c.fiat_unit || 'INR'}</div><div className="text-[10px] text-muted-foreground">{c.asset || 'USDT'}</div></TableCell>
