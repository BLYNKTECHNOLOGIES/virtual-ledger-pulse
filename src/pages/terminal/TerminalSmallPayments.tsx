...
import { AlertTriangle, ArrowLeftRight, CheckCircle2, Clock, CreditCard, FileWarning, MessageSquare, RefreshCw, Search, TimerReset } from 'lucide-react';
...
function getOrderStatusBadgeClass(status?: string | null) {
  ...
  if (s.includes('COMPLETED')) return 'border-success/30 text-success bg-success/5';
  if (s.includes('CANCEL') || s.includes('EXPIRED')) return 'border-destructive/30 text-destructive bg-destructive/5';
  if (s.includes('APPEAL') || s.includes('DISPUTE')) return 'border-warning/30 text-warning bg-warning/5';
...
export default function TerminalSmallPayments() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      altUpi: open.filter((c) => c.case_type === 'alternate_upi_needed' || c.has_pending_alt_upi_request).length,
