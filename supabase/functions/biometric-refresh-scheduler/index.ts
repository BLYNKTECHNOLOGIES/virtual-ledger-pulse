...
Deno.serve(async (req) => {
  ...
  const buildCommands = (serial: string) => {
    const now = Date.now();
    // NOTE: device clock-sync (SET TIME) is intentionally NOT queued. Device
    // clock skew is handled permanently at ingest via
    // hr_biometric_devices.clock_offset_minutes (+30 min correction).
    return [
