...
function buildSvg(args: {
  ...
  const drawRow = (label: string, valueLines: { text: string; size: number; bold?: boolean; color?: string }[], hasBorder = true) => {
    ...
    for (const v of valueLines) {
      line += `<text x="${W - 24}" y="${ly}" text-anchor="end" font-family="Arial, sans-serif" font-size="${v.size}" ${v.bold ? 'font-weight="600"' : ""} fill="${v.color || "#222"}">${escapeXml(v.text)}</text>`;
      ly += v.size + 2;
    }
    detailsSvg += `
      <text x="24" y="${y + 22}" font-family="Arial, sans-serif" font-size="13" fill="#888">${escapeXml(label)}</text>
      ${line}
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <g clip-path="url(#card)">
    ...
    <text x="${W / 2}" y="48" text-anchor="middle" font-family="Arial, sans-serif" font-size="13" fill="#ffffff" opacity="0.9">To ${escapeXml(args.toUpiId)}</text>
    <text x="${W / 2}" y="92" text-anchor="middle" font-family="Arial, sans-serif" font-size="32" font-weight="700" fill="#ffffff">${escapeXml(fmtINR(args.amount).replace(/\.00$/, ".00"))}</text>
    <rect x="${W / 2 - 60}" y="110" width="120" height="28" rx="14" ry="14" fill="rgba(255,255,255,0.22)"/>
    <text x="${W / 2}" y="129" text-anchor="middle" font-family="Arial, sans-serif" font-size="12" font-weight="600" fill="#ffffff">✓ Completed</text>
    <text x="${W / 2}" y="160" text-anchor="middle" font-family="Arial, sans-serif" font-size="12" fill="#ffffff" opacity="0.85">${escapeXml(dt)}</text>
    ${detailsSvg}
  </g>
</svg>`;
}
...
async function renderPng(svg: string): Promise<Uint8Array> {
  await ensureWasm();
  const resvg = new Resvg(svg, { fitTo: { mode: "width", value: 840 } }); // 2x for crispness
