-- First unlock the bank account balance temporarily, create transaction, then update balance

-- Temporarily unlock the balance
...
-- Manually update the bank account balance (since the trigger was disabled)
UPDATE bank_accounts 
SET balance = balance - 7200,
    updated_at = now(),
    balance_locked = true  -- Lock it back after update
WHERE id = 'b3978295-1b85-4b6c-bb26-ac1f2b14160c';

-- Recreate the trigger but make it work with locked accounts
...
BEGIN
  ...
  IF NEW.status = 'COMPLETED' AND (OLD IS NULL OR OLD.status != 'COMPLETED') AND NEW.bank_account_id IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM bank_transactions 
      WHERE reference_number = NEW.order_number AND category = 'Purchase'
    ) THEN
      ...
      -- Temporarily unlock, update balance, then lock again
