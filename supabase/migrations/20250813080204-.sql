-- Re-attempt: create triggers, then backfill balances without violating non-negative balance constraint
...
-- 3) Lock bank account balance after a transaction is created
DO $$
BEGIN
  IF NOT EXISTS (
...
--    while clamping to 0 to satisfy non-negative balance constraint
WITH totals AS (
  SELECT 
    bank_account_id,
    SUM(
