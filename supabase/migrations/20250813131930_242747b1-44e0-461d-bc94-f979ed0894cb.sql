-- Create a function that temporarily disables the balance lock trigger during manual purchase
CREATE OR REPLACE FUNCTION public.create_manual_purchase_bypass_locks(
  p_order_number TEXT,
...
DECLARE
  ...
  was_locked BOOLEAN;
BEGIN
  -- Check bank account balance and lock status first
  ...
  -- Temporarily unlock the account for this transaction
  ...
  -- Create bank transaction manually
  ...
  -- Restore the original lock status
  UPDATE public.bank_accounts 
