-- Create a simple function that directly updates settlement status without complex trigger manipulation
CREATE OR REPLACE FUNCTION update_settlement_status_simple(
  order_ids UUID[],
  batch_id TEXT,
...
BEGIN
  -- Loop through each order ID and update directly
  FOREACH order_id IN ARRAY order_ids
  LOOP
    BEGIN
      -- Simple direct UPDATE - just change the settlement fields
      UPDATE sales_orders 
      SET 
