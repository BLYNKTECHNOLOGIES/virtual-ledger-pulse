...
ALTER TABLE employees ADD COLUMN IF NOT EXISTS job_contract_signed boolean DEFAULT false;
...
BEGIN
  ...
  -- Get next sequence number for this department and year - use table alias to avoid ambiguity
  SELECT COALESCE(MAX(
    CASE 
...
-- Fix the employee_type constraint
...
CHECK (employee_type IN ('Full-time', 'Part-time', 'Contract', 'Intern', 'Consultant', 'Temporary', 'Freelancer'));