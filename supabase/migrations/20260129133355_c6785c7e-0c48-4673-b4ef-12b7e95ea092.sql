-- Fix delete_purchase_order_with_reversal: use correct column "amount" instead of "quantity"
-- and allow negative balances on reversal
...
BEGIN
  ...
  BEGIN
    ...
    -- 2. Reverse wallet transactions (use column "amount", allow negative)
    ...
    LOOP
      IF wallet_transaction_record.transaction_type IN ('STOCK_IN','PURCHASE','IN','CREDIT') THEN
        ...
        SET balance = balance - wallet_transaction_record.amount,
            updated_at = now()
