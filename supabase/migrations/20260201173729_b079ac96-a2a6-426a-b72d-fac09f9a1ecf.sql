-- Create enhanced manual purchase function with TDS handling and percentage-based platform fees
CREATE OR REPLACE FUNCTION public.create_manual_purchase_complete_v2(
  ...
  p_tds_option TEXT DEFAULT 'none', -- 'none', '1%', '20%'
...
BEGIN
  ...
  -- Calculate net credit quantity (quantity minus platform fee in USDT terms)
  ...
  -- For purchases, we deduct the net payable amount (after TDS)
  IF current_balance < v_net_payable_amount THEN
    RAISE EXCEPTION 'Insufficient bank balance. Available: %, Required: %', current_balance, v_net_payable_amount;
  END IF;
  ...
  -- Create the purchase order with TDS and fee fields populated
  ...
  -- Create bank transaction for net payable amount (after TDS deduction)
  ...
  IF p_credit_wallet_id IS NOT NULL THEN
    ...
    FROM public.wallets 
    WHERE id = p_credit_wallet_id AND is_active = true;
    
    IF v_wallet_balance IS NULL THEN
      RAISE EXCEPTION 'Credit wallet not found or inactive';
    END IF;
    
    -- Credit net quantity (after fee deduction) to wallet
  ...
  ELSE
    -- Create stock transaction to update product stock (non-USDT products)
