-- Fix the ambiguous column reference in the manual purchase function
...
BEGIN
  ...
  -- Check bank account balance (use table alias to avoid ambiguity)
  ...
  IF p_credit_wallet_id IS NOT NULL THEN
    -- Get current wallet balance using table alias
    ...
    -- Update wallet balance using table alias
    UPDATE public.wallets w
    SET current_balance = w.current_balance + v_net_credit_quantity,
        updated_at = NOW()
    WHERE w.id = p_credit_wallet_id;
    
  ELSE
    -- Create stock transaction for non-USDT products
