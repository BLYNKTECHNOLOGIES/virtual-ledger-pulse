-- Fix double wallet balance update bug in create_manual_purchase_complete_v2
-- The trigger already updates wallet balance when inserting into wallet_transactions
-- So the explicit UPDATE statement causes double crediting

DROP FUNCTION IF EXISTS public.create_manual_purchase_complete_v2(text,text,date,numeric,uuid,numeric,numeric,uuid,text,text,uuid,text,text,numeric,boolean,uuid);
...
BEGIN
  ...
  IF p_credit_wallet_id IS NOT NULL THEN
    ...
    -- The trigger update_wallet_balance_trigger will automatically update the wallet balance
    ...
    -- NOTE: REMOVED explicit wallet balance update - the trigger handles it automatically
    -- This was causing double crediting issue

    -- Platform fee transaction (if applicable) - recorded as separate transaction for visibility
    IF v_fee_amount > 0 THEN
      INSERT INTO public.wallet_transactions (
        ...
        v_wallet_balance + v_net_credit_quantity,
        v_wallet_balance + v_net_credit_quantity - v_fee_amount,
        p_created_by
      );
      -- NOTE: Trigger will handle the balance update for the fee debit as well
    END IF;
  ELSE
    ...
    SET current_stock_quantity = COALESCE(current_stock_quantity, 0) + p_quantity,
        updated_at = NOW()
  ...
  RETURN jsonb_build_object(
    ...
    'net_credit', v_net_credit_quantity
  );
END;
$$;