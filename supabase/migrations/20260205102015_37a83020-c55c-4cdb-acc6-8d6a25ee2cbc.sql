-- Update the create_manual_purchase_complete_v2 function to also save PAN to client record
-- when a PAN is provided and the client doesn't already have one
...
BEGIN
  ...
  IF p_pan_number IS NOT NULL AND p_pan_number != '' THEN
    -- Find client by name (case-insensitive) or phone
    ...
    -- Update client's PAN if found and not already set
...
-- Also backfill existing clients with PAN from their TDS records
-- Only update clients that don't have a PAN yet
UPDATE public.clients c
SET pan_card_number = t.pan_number,
    updated_at = NOW()
FROM (
  SELECT DISTINCT po.supplier_name, tr.pan_number
  FROM public.tds_records tr
  JOIN public.purchase_orders po ON tr.purchase_order_id = po.id
  WHERE tr.pan_number IS NOT NULL AND tr.pan_number != ''
) t
WHERE LOWER(c.name) = LOWER(t.supplier_name)
  AND (c.pan_card_number IS NULL OR c.pan_card_number = '');