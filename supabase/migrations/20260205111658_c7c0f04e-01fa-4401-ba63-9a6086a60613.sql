-- Fix: the function return type for transaction_date should be DATE not TIMESTAMPTZ
-- Also simplify the closing balance calculation for better performance
...
BEGIN
  ...
  WITH ordered_tx AS (
    SELECT 
      ...
      -- Cumulative sum of all transactions AFTER this one (since we're ordered descending)
      -- For the most recent transaction, this is 0
      -- For older transactions, this is the sum of more recent transactions
      COALESCE(
        SUM(
          CASE 
  ...
  SELECT 
    ...
    -- Closing balance = current balance - sum of transactions after this one
