...
BEGIN
  -- Prevent double-reversal using reversal_guards
  INSERT INTO reversal_guards (entity_type, entity_id)
  VALUES ('payment_gateway_settlement', p_settlement_id::text)
  ...
  IF v_settlement IS NULL THEN
    -- Clear stale guard
    DELETE FROM reversal_guards WHERE entity_type = 'payment_gateway_settlement' AND entity_id = p_settlement_id::text;
  ...
  LOOP
    -- Only restore if no existing pending settlement for this sales order
    ...
    -- Update sales order settlement_status back
    UPDATE sales_orders
  ...
  -- Delete INCOME bank transaction for this settlement
  ...
  -- Mark settlement as REVERSED (keep for audit trail)
