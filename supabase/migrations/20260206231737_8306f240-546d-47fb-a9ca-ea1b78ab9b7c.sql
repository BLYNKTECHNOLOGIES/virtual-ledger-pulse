...
-- Atomic function to reverse/delete a contra entry (bank transfer pair)
-- Uses reversal_guards to prevent double-reversal
...
BEGIN
  ...
  -- Build guard key and attempt insert (prevents double-reversal)
  v_guard_key := 'contra_delete_' || p_transfer_out_id::TEXT;
  
  -- Clear any stale guard for this ID first (half-failed previous attempt)
  DELETE FROM reversal_guards WHERE guard_key = v_guard_key;
  
  -- Insert fresh guard
  INSERT INTO reversal_guards (guard_key) VALUES (v_guard_key);
