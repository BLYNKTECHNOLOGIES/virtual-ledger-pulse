-- Fix create_manual_purchase_complete_v2: Add asset_code to wallet transactions
CREATE OR REPLACE FUNCTION public.create_manual_purchase_complete_v2(
  ...
  p_credit_wallet_id uuid DEFAULT NULL::uuid, p_tds_option text DEFAULT 'NO_TDS'::text,
  p_pan_number text DEFAULT NULL::text, p_fee_percentage numeric DEFAULT NULL::numeric,
  p_is_off_market boolean DEFAULT false, p_created_by uuid DEFAULT NULL::uuid
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Fix process_sales_order_wallet_deduction: Add p_asset_code parameter
CREATE OR REPLACE FUNCTION public.process_sales_order_wallet_deduction(
  sales_order_id uuid, wallet_id uuid, usdt_amount numeric, p_asset_code text DEFAULT 'USDT'
)
RETURNS boolean
LANGUAGE plpgsql
SET search_path TO 'public'
...
-- Fix existing corrupted BTC wallet transaction (wrong asset_code USDT -> BTC)
UPDATE public.wallet_transactions 
SET asset_code = 'BTC'
WHERE reference_id = '4f90519e-6d47-43c4-8206-9278927c788f' 
