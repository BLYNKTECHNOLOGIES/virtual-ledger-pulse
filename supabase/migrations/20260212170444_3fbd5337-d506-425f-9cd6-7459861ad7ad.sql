...
-- Reset seeded positions to zero since existing conversions predate WAC system
-- Positions will build organically from new approved conversions
UPDATE public.wallet_asset_positions
SET qty_on_hand = 0, cost_pool_usdt = 0, avg_cost_usdt = 0;

-- Phase 2: WAC-aware approve_product_conversion RPC
...
BEGIN
  ...
  IF v_conv.side = 'BUY' THEN
    -- Net qty = gross qty minus fee (if fee is in bought asset)
  ...
  IF v_conv.side = 'BUY' THEN
    ...
    -- Post wallet transactions (triggers handle balance updates)
    ...
    SET qty_on_hand = v_new_qty, cost_pool_usdt = v_new_pool, avg_cost_usdt = v_new_avg, updated_at = now()
    WHERE id = v_pos.id;
    ...
    -- Update conversion record with computed fields
