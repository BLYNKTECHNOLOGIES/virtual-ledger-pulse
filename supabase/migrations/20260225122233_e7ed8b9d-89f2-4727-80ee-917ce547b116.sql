...
BEGIN
  ...
  -- 3. Nullify UUID-typed FK references (preserve all history/usernames)
  ...
  UPDATE public.erp_product_conversions SET created_by = NULL WHERE created_by = _tid;
  ...
  UPDATE public.system_action_logs SET user_id = NULL WHERE user_id = _tid;
  ...
  -- 4. Nullify TEXT-typed FK references (cast uuid to text)
  UPDATE public.ad_action_logs SET user_id = 'deleted' WHERE user_id = _tid_text;
  ...
  -- 5. Nullify self-references in users table
