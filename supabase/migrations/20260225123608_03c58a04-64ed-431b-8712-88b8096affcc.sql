...
BEGIN
  ...
  -- DELETE audit logs with NOT NULL FK to users (placeholder won't work)
  DELETE FROM public.terminal_assignment_audit_logs WHERE performed_by = p_user_id OR target_user_id = p_user_id;

  -- NOT NULL FK columns that DON'T reference users table directly (no FK check on placeholder)
  ...
  UPDATE public.ad_rest_timer SET started_by = NULL WHERE started_by = p_user_id;
  UPDATE public.bank_accounts SET dormant_by = NULL WHERE dormant_by = p_user_id;
  UPDATE public.bank_cases SET created_by = NULL WHERE created_by = p_user_id;
  UPDATE public.bank_cases SET assigned_to = NULL WHERE assigned_to = p_user_id;
  UPDATE public.bank_cases SET resolved_by = NULL WHERE resolved_by = p_user_id;
  UPDATE public.account_investigations SET assigned_to = NULL WHERE assigned_to = p_user_id;
  UPDATE public.closed_bank_accounts SET closed_by = NULL WHERE closed_by = p_user_id;
  UPDATE public.employee_offboarding SET initiated_by = NULL WHERE initiated_by = p_user_id;

  -- TEXT columns
  UPDATE public.ad_action_logs SET user_id = _placeholder::text WHERE user_id = p_user_id::text;
  UPDATE public.terminal_sessions SET user_id = _placeholder::text WHERE user_id = p_user_id::text;
