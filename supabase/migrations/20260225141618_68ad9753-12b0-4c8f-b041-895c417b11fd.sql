...
BEGIN
  ...
  -- Set dust threshold based on asset type
  ...
  ELSIF v_conv.side = 'SELL' THEN
    ...
    -- CRITICAL FIX: Use ledger SUM as source of truth instead of cached wallet_asset_balances
    -- The cached balance can drift due to concurrent transactions or trigger timing
    ...
    IF COALESCE(v_actual_balance, 0) < v_qty_net THEN
      ...
      ELSE
        ...
        RETURN jsonb_build_object('success', false, 'error',
          'Insufficient balance. Available (ledger): ' || COALESCE(v_actual_balance, 0) || ', Required: ' || v_qty_net);
      END IF;
    -- AUTO-INCLUDE DUST: if balance exceeds required by tiny amount, sell ALL
    ...
    -- SAFETY: absolute guard - never allow negative result
    IF v_actual_balance - v_qty_net < -0.000000001 THEN
      DELETE FROM reversal_guards WHERE entity_type = 'erp_conversion' AND entity_id = p_conversion_id AND action = 'approve';
      RETURN jsonb_build_object('success', false, 'error',
        'Safety check failed: approval would result in negative balance. Available: ' || v_actual_balance || ', Selling: ' || v_qty_net);
    END IF;
