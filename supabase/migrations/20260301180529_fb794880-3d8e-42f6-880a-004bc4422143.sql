...
-- Fix create_manual_purchase_complete_v2: skip balance check for CREDIT accounts
...
BEGIN
  ...
  -- Get bank balance AND account type
  ...
  IF p_credit_wallet_id IS NOT NULL THEN
    ...
    INSERT INTO public.wallet_transactions (
      ...
      description, running_balance, asset_code
    ) VALUES (
      p_credit_wallet_id, 'CREDIT', v_net_credit_quantity, 'PURCHASE', v_purchase_order_id,
      'Purchase credit - ' || p_order_number,
      COALESCE(v_wallet_balance, 0) + v_net_credit_quantity,
      v_product_code
    );

    v_wallet_balance_after_credit := COALESCE(v_wallet_balance, 0) + v_net_credit_quantity;
    UPDATE public.wallets SET current_balance = v_wallet_balance_after_credit WHERE id = p_credit_wallet_id;
  END IF;

  RETURN jsonb_build_object('success', true, 'purchase_order_id', v_purchase_order_id);
END;
$$;
