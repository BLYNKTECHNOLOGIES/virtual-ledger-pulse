...
-- Fix sync_p2p_order to enforce forward-only status progression
-- This prevents stale active-order polls from reverting terminal statuses (e.g. COMPLETED → BUYER_PAYED)
...
BEGIN
  ...
  -- Status ranking: higher = more advanced in lifecycle
  -- Only allow status to move forward, never backward
  ...
  -- Use the more advanced status (forward-only)
