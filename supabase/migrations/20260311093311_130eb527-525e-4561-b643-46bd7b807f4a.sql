-- Update approve_product_conversion with safety clamps for avg_cost
-- The root cause: when cost_pool goes negative due to rounding/edge cases,
-- the avg_cost becomes wildly negative, then subsequent SELLs multiply by that negative avg
-- producing enormous cost_out values that overflow NUMERIC(20,9)
...
BEGIN
  ...
  -- SAFETY CLAMP: If avg_cost or cost_pool is corrupted (negative or absurdly large), reset to execution rate
  IF v_pos.avg_cost_usdt < 0 OR v_pos.avg_cost_usdt > 999999 OR v_pos.cost_pool_usdt < -1 THEN
    ...
    SET avg_cost_usdt = GREATEST(v_exec_rate, 0),
        ...
        updated_at = now()
    WHERE id = v_pos.id;
    -- Re-read the corrected position
