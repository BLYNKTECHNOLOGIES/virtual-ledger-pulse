...
-- Step 1: Make created_by nullable on erp_product_conversions (financial data must never be deleted)
ALTER TABLE public.erp_product_conversions ALTER COLUMN created_by DROP NOT NULL;

-- Step 2: Make user_id nullable on system_action_logs (audit trail must be preserved)
ALTER TABLE public.system_action_logs ALTER COLUMN user_id DROP NOT NULL;

-- Step 3: Make user_id nullable on ad_action_logs (has user_name column for preservation)
ALTER TABLE public.ad_action_logs ALTER COLUMN user_id DROP NOT NULL;

-- Step 4: Make user_id nullable on chat_message_senders
ALTER TABLE public.chat_message_senders ALTER COLUMN user_id DROP NOT NULL;

-- Step 5: Add actor_name columns to key tables for name preservation after user deletion
DO $$
BEGIN
  -- system_action_logs: add user_name if not exists
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'system_action_logs' AND column_name = 'user_name' AND table_schema = 'public') THEN
    ALTER TABLE public.system_action_logs ADD COLUMN user_name text;
  END IF;
  
  -- erp_product_conversions: add created_by_name if not exists  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'erp_product_conversions' AND column_name = 'created_by_name' AND table_schema = 'public') THEN
    ALTER TABLE public.erp_product_conversions ADD COLUMN created_by_name text;
  END IF;
END $$;

-- Step 6: Recreate the delete function with data preservation
