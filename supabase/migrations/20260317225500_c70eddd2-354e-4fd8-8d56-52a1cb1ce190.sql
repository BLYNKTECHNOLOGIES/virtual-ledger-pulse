  ...
  ELSIF v_conv.side = 'SELL' THEN
    ...
    IF COALESCE(v_actual_balance, 0) < v_qty_net THEN
      IF COALESCE(v_actual_balance, 0) >= v_qty_net * 0.98 AND COALESCE(v_actual_balance, 0) > 0 THEN
        ...
        -- FIX: Do NOT proportionally reduce v_gross_usdt.
        -- Binance already executed the full trade and returned the full USDT quote amount.
        -- Only the asset debit quantity should be capped, NOT the USDT proceeds.
        -- v_gross_usdt remains unchanged (original quote amount from Binance).
      ELSE
