-- B25 Fix: Correct stock_transactions INSERT column names in create_manual_purchase_with_split_payments
CREATE OR REPLACE FUNCTION create_manual_purchase_with_split_payments(
  p_order_number TEXT,
...
BEGIN
  ...
  -- Validate each bank has sufficient balance (skip for CREDIT accounts)
  FOR v_split IN SELECT * FROM jsonb_array_elements(p_payment_splits) LOOP
  ...
  IF p_credit_wallet_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- FIXED: Use correct column names (was unit_cost/total_cost/reference_type/reference_id/notes)
    INSERT INTO public.stock_transactions (
      product_id, transaction_type, quantity, unit_price, total_amount,
      reference_number, reason, supplier_customer_name, transaction_date, created_by
    ) VALUES (
      p_product_id, 'PURCHASE', p_quantity, p_unit_price, p_total_amount,
      p_order_number, 'Manual purchase [Split Payment]', p_supplier_name, p_order_date, p_created_by
    );
  END IF;
