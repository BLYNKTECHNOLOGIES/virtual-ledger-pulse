-- S2: Drop the dangerous bypass RPC — it's unused and a security risk
DROP FUNCTION IF EXISTS update_settlement_bypass_all_triggers(uuid[], text, timestamptz);

-- S1: Create a daily-callable function that flags stale pending settlements (>7 days)
...
BEGIN
  ...
  LOOP
    -- Create an ERP task if one doesn't already exist for this order
    IF NOT EXISTS (
