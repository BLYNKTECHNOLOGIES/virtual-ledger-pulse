-- CLEANUP: Fix BASIC component_type from 'allowance' to 'earning'
-- Also update fn_generate_payroll to handle 'earning' type on the earnings side

UPDATE hr_salary_components SET component_type = 'earning' WHERE code = 'BASIC';

-- Update fn_generate_payroll to treat 'earning' same as 'allowance' (earnings side)
...
BEGIN
  ...
  -- Also clean up loan repayments and deposit transactions from previous generation
  ...
  LOOP
    ...
    IF v_emp.salary_structure_template_id IS NOT NULL AND v_total_salary > 0 THEN
      ...
      LOOP
        ...
        -- UPDATED: 'earning' and 'allowance' both go to earnings side
