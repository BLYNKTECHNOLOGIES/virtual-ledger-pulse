-- Migration 1: Fix Payroll Engine Bugs
-- Bug 1: 'draft' → 'generated' (trigger only allows generated/paid/cancelled)
-- Bug 2: Conditional status update
-- Bug 3: Loan sync trigger uses wrong column names
-- Bug 4: Use salary_template_id instead of salary_structure_template_id
-- Bug 5: present_days INTEGER → NUMERIC for half-days
-- Bug 6: Use fn_calculate_working_days instead of hardcoded Sunday
-- Bug 7: Add employer_contributions JSONB column

-- Fix 5: ALTER present_days to numeric
ALTER TABLE hr_payslips ALTER COLUMN present_days TYPE numeric USING present_days::numeric;

-- Fix 7: Add employer_contributions column
ALTER TABLE hr_payslips ADD COLUMN IF NOT EXISTS employer_contributions jsonb DEFAULT '{}'::jsonb;

-- Fix 4: Sync existing data
UPDATE hr_employees 
SET salary_template_id = salary_structure_template_id 
WHERE salary_template_id IS NULL 
  AND salary_structure_template_id IS NOT NULL;

-- Fix 1,2,4,6,7: Replace fn_generate_payroll
...
-- Fix 3: Loan sync trigger — fix wrong column names
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    SET outstanding_balance = COALESCE(
      (SELECT balance_after FROM hr_loan_repayments 
       WHERE loan_id = OLD.loan_id ORDER BY repayment_date DESC, created_at DESC LIMIT 1),
      amount
    ),
    status = CASE 
      WHEN COALESCE(
