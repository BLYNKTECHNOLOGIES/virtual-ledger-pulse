...
BEGIN
  ...
  -- Check if caller is ERP Super Admin (bypasses all terminal guards)
  SELECT public.has_role(v_caller_id, 'Super Admin') INTO v_is_erp_super_admin;

  -- Get caller's minimum hierarchy level from terminal roles
  ...
  -- Super Admin (terminal level < 0 OR ERP Super Admin) bypasses all checks
