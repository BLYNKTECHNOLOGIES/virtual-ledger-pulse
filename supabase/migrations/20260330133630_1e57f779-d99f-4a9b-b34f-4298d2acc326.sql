CREATE OR REPLACE FUNCTION public.create_manual_purchase_complete_v2(
  ...
  p_deduction_bank_account_id UUID DEFAULT NULL,
...
BEGIN
  ...
  -- Fixed: removed non-existent 'order_status' column
  INSERT INTO public.purchase_orders (
    order_number, supplier_name, order_date, description, total_amount, contact_number,
    status, total_paid, bank_account_id, tds_applied, pan_number,
    ...
    p_contact_number, 'COMPLETED', v_net_payable_amount, p_bank_account_id,
  ...
  IF v_tds_rate > 0 THEN
    INSERT INTO public.tds_records (
      supplier_name, pan_number, tds_rate, tds_amount, total_amount,
      net_payable_amount, financial_year, purchase_order_id, bank_account_id
    ) VALUES (
      p_supplier_name, p_pan_number, v_tds_rate, v_tds_amount,
      p_total_amount, v_net_payable_amount, v_financial_year,
      v_purchase_order_id, p_bank_account_id
    );
  END IF;

  INSERT INTO public.stock_transactions (
    product_id, transaction_type, quantity, unit_price, total_amount,
    reference_number, reason, warehouse_id
  ) VALUES (
    p_product_id, 'IN', v_net_credit_quantity, p_unit_price,
    v_net_credit_quantity * p_unit_price, p_order_number,
    'Manual Purchase - ' || p_supplier_name, p_credit_wallet_id
  );

  INSERT INTO public.wallet_transactions (
    wallet_id, transaction_type, amount, description,
    reference_id, balance_before, balance_after, asset_code
  ) VALUES (
    p_credit_wallet_id, 'CREDIT', v_net_credit_quantity,
    'Manual Purchase - ' || p_order_number || ' (' || p_supplier_name || ')',
    v_purchase_order_id, 0, 0, v_product_code
  );

  INSERT INTO public.purchase_order_payment_splits (
    purchase_order_id, bank_account_id, amount, payment_date, status, notes
  ) VALUES (
    v_purchase_order_id, p_bank_account_id, v_net_payable_amount,
    p_order_date, 'COMPLETED',
    'Auto-created from manual purchase entry'
  );
