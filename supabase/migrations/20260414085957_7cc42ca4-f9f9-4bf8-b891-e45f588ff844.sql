...
BEGIN
  ...
  -- Removed: email_verification_tokens and password_reset_tokens (tables no longer exist)
  ...
  UPDATE public.sales_order_sync_reviews SET reviewed_by = _user_name WHERE reviewed_by = _tid;
  UPDATE public.bank_cases SET assigned_to = _user_name WHERE assigned_to = _tid;
  UPDATE public.bank_cases SET resolved_by = _user_name WHERE resolved_by = _tid;
  UPDATE public.bank_cases SET created_by = _user_name WHERE created_by = _tid;
  ...
  UPDATE public.task_comments SET user_id = NULL WHERE user_id = p_user_id;

  DELETE FROM public.users WHERE id = p_user_id;

  RETURN json_build_object('success', true, 'deleted_user', _full_name);
END;
$function$;
