...
AS $$
  SELECT
    ...
    (wt.balance_before
      + CASE WHEN wt.balance_after >= wt.balance_before
             ...
             ELSE -ABS(COALESCE(wt.amount,0)) END
    ) AS expected_running_total,
    'ARITHMETIC'::text AS break_type,
    format('|balance_delta| = %s but |amount| = %s',
           ABS(wt.balance_after - wt.balance_before), ABS(COALESCE(wt.amount,0))) AS details
  ...
  WHERE LOWER(TRIM(w.wallet_name)) <> 'balance adjustment wallet'
    ...
    AND ABS(ABS(wt.balance_after - wt.balance_before) - ABS(COALESCE(wt.amount,0))) > 0.00000001
