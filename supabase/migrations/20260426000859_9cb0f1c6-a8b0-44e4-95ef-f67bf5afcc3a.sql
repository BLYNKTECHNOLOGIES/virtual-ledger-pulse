...
BEGIN
  WITH role_users AS (
    ...
    SELECT ru.user_id,
      ...
      count(DISTINCT (pe.paid_at AT TIME ZONE 'Asia/Kolkata')::date)::int AS payer_active_days,
      mode() WITHIN GROUP (ORDER BY extract(hour from pe.paid_at AT TIME ZONE 'Asia/Kolkata')) AS payer_peak_hour,
    ...
    SELECT ru.user_id,
      ...
      count(DISTINCT (toa.created_at AT TIME ZONE 'Asia/Kolkata')::date)::int AS operator_active_days,
      mode() WITHIN GROUP (ORDER BY extract(hour from toa.created_at AT TIME ZONE 'Asia/Kolkata')) AS operator_peak_hour,
    ...
    SELECT ru.user_id,
      ...
      CASE
        ...
        ELSE 'operator'
      END AS role_type,
      coalesce(ps.payment_count,0) payment_count, coalesce(ps.lock_count,0) lock_count, coalesce(ps.completed_locks,0) completed_locks,
      coalesce(ps.active_locks,0) active_locks, coalesce(ps.stale_locks,0) stale_locks, coalesce(ps.matched_payment_orders,0) matched_payment_orders,
      coalesce(ps.cancelled_after_action,0) cancelled_after_action, coalesce(ps.appeal_after_action,0) appeal_after_action, coalesce(ps.payment_volume,0) payment_volume,
      coalesce(ps.payer_active_days,0) payer_active_days, ps.payer_peak_hour, ps.payer_times,
      coalesce(os.assignment_count,0) assignment_count, coalesce(os.completed_assignments,0) completed_assignments, coalesce(os.cancelled_assignments,0) cancelled_assignments,
      coalesce(os.active_assignments,0) active_assignments, coalesce(os.stale_assignments,0) stale_assignments, coalesce(os.appeal_assignments,0) appeal_assignments,
      coalesce(os.matched_assignment_orders,0) matched_assignment_orders, coalesce(os.buy_count,0) buy_count, coalesce(os.sell_count,0) sell_count,
      coalesce(os.assignment_volume,0) assignment_volume, coalesce(os.operator_active_days,0) operator_active_days, os.operator_peak_hour, os.operator_times,
      aq.action_log_count
    FROM role_users ru
    ...
    SELECT *,
      CASE WHEN role_type IN ('payer','hybrid') THEN payment_count ELSE assignment_count END AS work_count,
      CASE WHEN role_type IN ('payer','hybrid') THEN payment_volume ELSE assignment_volume END AS total_volume,
      CASE WHEN role_type IN ('payer','hybrid') THEN completed_locks ELSE completed_assignments END AS completed_count,
      CASE WHEN role_type IN ('payer','hybrid') THEN stale_locks ELSE stale_assignments END AS stale_count,
      CASE WHEN role_type IN ('payer','hybrid') THEN cancelled_after_action ELSE cancelled_assignments END AS cancelled_count,
      ...
      CASE WHEN role_type IN ('payer','hybrid') AND payment_count > 0 THEN round((matched_payment_orders::numeric/payment_count::numeric)*100,2)
           WHEN role_type NOT IN ('payer','hybrid') AND assignment_count > 0 THEN round((matched_assignment_orders::numeric/assignment_count::numeric)*100,2) END AS history_coverage,
      CASE WHEN role_type IN ('payer','hybrid') AND greatest(lock_count, payment_count) > 0 THEN round((completed_locks::numeric/greatest(lock_count,payment_count)::numeric)*100,2)
           WHEN role_type NOT IN ('payer','hybrid') AND assignment_count > 0 THEN round((completed_assignments::numeric/assignment_count::numeric)*100,2) END AS completion_rate,
      greatest(payer_active_days, operator_active_days) AS active_days,
      coalesce(payer_peak_hour, operator_peak_hour) AS peak_hour
    FROM base
  ), scored AS (
    SELECT *,
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    'appeal_count', appeal_after_action+appeal_assignments, 'payment_count', payment_count, 'lock_count', lock_count, 'completed_locks', completed_locks,
    'active_locks', active_locks, 'stale_locks', stale_locks, 'assignment_count', assignment_count, 'completed_assignments', completed_assignments,
    'active_assignments', active_assignments, 'stale_assignments', stale_assignments, 'buy_count', buy_count, 'sell_count', sell_count,
    'peak_hour', peak_hour, 'history_coverage', history_coverage,
    'source_counts', jsonb_build_object('matched_payment_orders', matched_payment_orders, 'matched_assignment_orders', matched_assignment_orders, 'action_logs', action_log_count, 'timed_payer_events', coalesce(cardinality(payer_times),0), 'timed_operator_events', coalesce(cardinality(operator_times),0)),
