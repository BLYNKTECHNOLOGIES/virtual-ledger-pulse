...
BEGIN
  ...
  -- Terminal access/session cleanup: remove every active Terminal identity and access surface.
  ...
  -- Terminal assignment cleanup: remove deleted users from assignment routing.
  ...
  -- Wallet ledger: open documented escape hatch with the CORRECT flag, then close it.
  ...
  -- Bank ledger: same pattern with the CORRECT flag.
...
EXCEPTION WHEN OTHERS THEN
  -- Defensive: ensure escape hatches are closed even on failure.
  PERFORM set_config('app.allow_ledger_mutation', 'off', true);
  PERFORM set_config('app.allow_bank_ledger_mutation', 'off', true);
