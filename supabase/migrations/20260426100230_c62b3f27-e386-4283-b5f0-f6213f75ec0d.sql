...
BEGIN
    ...
    WHERE h.order_number = p_order_number
      AND (
        ...
        OR (
          ...
          AND upper(coalesce(h.order_detail_raw->>'complaintStatus', '')) NOT IN ('0', '3', 'CLOSED', 'RESOLVED', 'CANCELLED', 'CANCELED', '2')
        )
      )
  ...
  IF v_existing_status IS DISTINCT FROM v_effective_status THEN
    INSERT INTO public.terminal_appeal_case_events (case_id, event_type, actor_user_id, note, metadata)
    VALUES (v_case_id, 'status_changed', v_actor, p_request_reason, jsonb_build_object('from_status', v_existing_status, 'to_status', v_effective_status, 'binance_status', v_effective_binance_status));
  END IF;
...
WITH stale_final_appeals AS (
  SELECT c.id,
    public.terminal_order_final_appeal_status(public.get_authoritative_terminal_order_status(c.order_number)) AS final_appeal_status,
    public.get_authoritative_terminal_order_status(c.order_number) AS final_order_status
  ...
  WHERE c.status NOT IN ('resolved', 'closed', 'cancelled')
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM public.binance_order_history h
      WHERE h.order_number = c.order_number
        AND (
          public.terminal_order_is_appeal_status(h.raw_data->>'orderStatus')
          OR public.terminal_order_is_appeal_status(h.raw_data->>'order_status')
          OR public.terminal_order_is_appeal_status(h.order_status)
...
SET status = COALESCE(s.final_appeal_status, 'cancelled'),
    binance_status = s.final_order_status,
    response_timer_minutes = NULL,
    response_due_at = NULL,
    response_timer_set_by = NULL,
    response_timer_set_at = NULL,
    updated_at = now()
FROM stale_final_appeals s
WHERE c.id = s.id;