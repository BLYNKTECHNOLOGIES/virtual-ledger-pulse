...
WITH should_be_active AS (
  SELECT c.id,
    CASE WHEN c.response_timer_set_at IS NOT NULL OR c.response_due_at IS NOT NULL THEN 'respond_by_set' ELSE 'under_appeal' END AS restored_status
  FROM public.terminal_appeal_cases c
  WHERE c.status IN ('resolved', 'closed', 'cancelled')
