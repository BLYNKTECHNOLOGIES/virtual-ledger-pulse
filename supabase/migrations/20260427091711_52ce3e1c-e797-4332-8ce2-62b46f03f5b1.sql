...
AS $function$
  SELECT upper(coalesce(p_status, '')) = '8'
      OR upper(coalesce(p_status, '')) LIKE '%APPEAL%'
  ...
  SELECT EXISTS (
    ...
    WHERE h.order_number = p_order_number
      AND (
        ...
        OR (
          NOT public.terminal_order_is_final_status(h.order_status::text)
          AND coalesce(h.order_detail_raw->>'complaintReason', '') <> ''
        )
      )
  )
$function$;
...
WHERE c.source = 'binance_status'
  ...
  AND NOT public.terminal_order_has_active_appeal_evidence(c.order_number);