...
BEGIN
  ...
  -- Determine the asset currently sitting in the wallet ledger for this order
  SELECT wt.asset_code INTO v_old_asset_code
    FROM public.wallet_transactions wt
   WHERE wt.reference_id = p_order_id
     AND wt.reference_type IN ('PURCHASE','PURCHASE_ORDER')
  ...
  -- NEW: detect a Product/Asset change (e.g. BTC -> USDT) even if wallet/qty/amount are unchanged
  v_asset_changed := (v_old_asset_code IS NOT NULL
                      AND UPPER(TRIM(v_old_asset_code)) IS DISTINCT FROM UPPER(TRIM(COALESCE(p_product_code,'USDT'))));
  ...
  IF v_wallet_changed OR v_quantity_changed OR v_amount_changed OR v_asset_changed THEN
    FOR r IN
  ...
  IF v_quantity_changed OR v_amount_changed OR v_asset_changed THEN
  ...
  RETURN jsonb_build_object(
    ...
    'asset_changed', v_asset_changed,
    'old_asset_code', v_old_asset_code,
