CREATE OR REPLACE FUNCTION public.create_bank_transfer(
  ...
  p_from_sub_ledger_id uuid DEFAULT NULL::uuid,
  p_to_sub_ledger_id uuid DEFAULT NULL::uuid
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT * INTO v_to_account   FROM public.bank_accounts WHERE id = p_to_account_id FOR UPDATE;
  ...
  INSERT INTO public.bank_transactions (
    ...
    reference_number, related_account_name, created_by, sub_ledger_id
  )
  VALUES (
    ...
    p_created_by,
    CASE WHEN v_to_account.account_type = 'CREDIT' THEN p_to_sub_ledger_id ELSE NULL END
  );

  INSERT INTO public.bank_transactions (
    id, bank_account_id, transaction_type, amount, description, transaction_date,
    reference_number, related_account_name, related_transaction_id, created_by, sub_ledger_id
  )
  VALUES (
    ...
    v_in_id,
    p_created_by,
    CASE WHEN v_from_account.account_type = 'CREDIT' THEN p_from_sub_ledger_id ELSE NULL END
  );
