CREATE OR REPLACE FUNCTION public.get_transactions_with_closing_balance(p_bank_account_id uuid DEFAULT NULL::uuid, p_transaction_type text DEFAULT NULL::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_start_date date DEFAULT NULL::date, p_end_date date DEFAULT NULL::date)
 RETURNS TABLE(id uuid, transaction_date date, created_at timestamp with time zone, bank_account_id uuid, account_name text, bank_name text, transaction_type text, amount numeric, description text, category text, reference_number text, related_account_name text, closing_balance numeric, balance_before numeric, sequence_no bigint, is_reversed boolean, reverses_transaction_id uuid, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    bt.balance_after  AS closing_balance,
    bt.balance_before AS balance_before,
