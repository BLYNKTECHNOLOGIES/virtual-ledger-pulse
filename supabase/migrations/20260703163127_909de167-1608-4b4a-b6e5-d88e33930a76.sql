...
BEGIN
  IF NEW.transaction_type IN ('DEBIT', 'TRANSFER_OUT', 'FEE') THEN
    ...
    -- The Balance Adjustment Wallet is an audit/contra bucket that is
    -- explicitly permitted to go negative; skip the balance check for it.
    IF LOWER(TRIM(wallet_name_var)) = 'balance adjustment wallet' THEN
      RETURN NEW;
    END IF;
