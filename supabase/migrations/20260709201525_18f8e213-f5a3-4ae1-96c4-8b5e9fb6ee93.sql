CREATE OR REPLACE FUNCTION public.phase3a_demerge()
 RETURNS TABLE(out_batch_id uuid, clients_created integer, skipped_same_identity integer, skipped_name_collision integer, sales_moved integer, purchases_moved integer, nicknames_moved integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    -- Default Binance handles (P2P-*/User-*) are now INCLUDED: DB analysis proved
    -- they are a perfect 1:1 bijection with userNo and are the only identity
    -- string for many counterparties. The global-uniqueness guard below (u = 1)
    -- still prevents shared/ambiguous nicknames from being reassigned.
    FOR n IN
      ...
      WHERE bn.client_id = r.client_uuid
        ...
        AND lower(btrim(bn.nickname)) IN (
          SELECT nk FROM (
            ...
            WHERE coalesce(btrim(c2.nickname),'')<>''
