-- Audit table
CREATE TABLE IF NOT EXISTS public.client_userno_merge_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  cp_userno text NOT NULL,
  survivor_id uuid NOT NULL,
  survivor_name text,
  merged_client_id uuid NOT NULL,
  merged_client_name text,
  canonical_name text,
  created_at timestamptz NOT NULL DEFAULT now()
);
GRANT SELECT ON public.client_userno_merge_log TO authenticated;
GRANT ALL ON public.client_userno_merge_log TO service_role;
ALTER TABLE public.client_userno_merge_log ENABLE ROW LEVEL SECURITY;
CREATE POLICY "authenticated read userno merge log"
  ON public.client_userno_merge_log FOR SELECT TO authenticated USING (true);

-- Merge function
CREATE OR REPLACE FUNCTION public.merge_shared_userno_clients(p_dry_run boolean DEFAULT true)
RETURNS TABLE(cp_userno text, survivor_id uuid, canonical_name text, merged_ids uuid[], merged_count int)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    -- canonical verified name = most frequent verified name on this account's orders
    ...
    -- survivor = most orders, then has phone, verified, not deleted, oldest
    ...
    LOOP
      ...
      IF NOT p_dry_run THEN
        ...
        UPDATE client_binance_nicknames SET client_id = v_survivor WHERE client_id = v_loser.id;
        ...
        -- verified names: drop dup (client_id, verified_name) then move
        ...
        -- userno registry: cp_userno is globally unique -> drop loser rows
    ...
    IF NOT p_dry_run THEN
      -- ensure the userno registry row points to the survivor
      UPDATE client_binance_usernos SET client_id = v_survivor WHERE cp_userno = r_uno.user_no;
      ...
      WHERE NOT EXISTS (SELECT 1 FROM client_binance_usernos WHERE cp_userno = r_uno.user_no);

      -- set survivor's name to canonical verified name
      IF v_canon IS NOT NULL THEN
        UPDATE clients SET name = v_canon, updated_at = now() WHERE id = v_survivor;
      END IF;
    END IF;

    cp_userno := r_uno.user_no;
    survivor_id := v_survivor;
    canonical_name := v_canon;
    merged_ids := v_merged;
    merged_count := COALESCE(array_length(v_merged, 1), 0);
    RETURN NEXT;
  END LOOP;
END;
$$;