...
BEGIN
  ...
  -- IDEMPOTENCY: reverse loan repayments from any previous run of this payroll
  -- before deleting them, so re-running does not double-charge EMIs.
  UPDATE hr_loans l
  ...
  -- IDEMPOTENCY: reverse deposit transactions from any previous run of this payroll
  -- before deleting them, so collected_amount / current_balance are restored.
  FOR v_dep_txn IN
    SELECT deposit_id, transaction_type, amount
    FROM hr_deposit_transactions
    WHERE payroll_run_id = p_payroll_run_id
  LOOP
    IF v_dep_txn.transaction_type = 'collection' THEN
      UPDATE hr_employee_deposits
      SET collected_amount = GREATEST(0, collected_amount - v_dep_txn.amount),
          ...
          updated_at = now()
      WHERE id = v_dep_txn.deposit_id;
    ELSIF v_dep_txn.transaction_type = 'replenishment' THEN
      UPDATE hr_employee_deposits
      SET current_balance = GREATEST(0, current_balance - v_dep_txn.amount),
          updated_at = now()
      WHERE id = v_dep_txn.deposit_id;
    ELSIF v_dep_txn.transaction_type = 'penalty_deduction' THEN
      -- stored as negative amount; reverse by subtracting (i.e. adding |amount|)
      UPDATE hr_employee_deposits
      SET current_balance = current_balance - v_dep_txn.amount,
          updated_at = now()
      WHERE id = v_dep_txn.deposit_id;
    END IF;
  END LOOP;
  ...
  FOR v_emp IN
    SELECT id, first_name, last_name, total_salary, basic_salary, 
           salary_template_id, filing_status_id
    FROM hr_employees
  ...
  LOOP
    ...
    -- LEAVE CLIPPING + is_paid HONORING:
    -- Clip each leave request to the pay period, and split by leave-type is_paid flag.
    -- Paid leave counts toward present_days; unpaid leave does not (falls through to LOP).
    v_leave_days := 0;
    v_unpaid_leave_days := 0;
    SELECT
      COALESCE(SUM(
        CASE WHEN COALESCE(lt.is_paid, true) THEN
          (LEAST(lr.end_date, v_run.pay_period_end) - GREATEST(lr.start_date, v_run.pay_period_start) + 1)::numeric
          * (lr.total_days / NULLIF((lr.end_date - lr.start_date + 1)::numeric, 0))
        ELSE 0 END
      ), 0),
      COALESCE(SUM(
        CASE WHEN COALESCE(lt.is_paid, true) = false THEN
          (LEAST(lr.end_date, v_run.pay_period_end) - GREATEST(lr.start_date, v_run.pay_period_start) + 1)::numeric
          * (lr.total_days / NULLIF((lr.end_date - lr.start_date + 1)::numeric, 0))
        ELSE 0 END
      ), 0)
    ...
    IF v_present_days = 0 AND v_leave_days = 0 AND v_unpaid_leave_days = 0 THEN
      -- No attendance record at all and no leave: assume full presence (legacy behavior).
      IF NOT EXISTS (
    ...
    IF v_emp.salary_template_id IS NOT NULL AND v_total_salary > 0 THEN
      ...
      LOOP
        ...
        IF v_item.component_type IN ('allowance', 'earning') THEN
          -- FIX: Do NOT prorate earnings by attendance ratio. LOP deduction below
          -- is the single source of truth for absence-based pay reduction.
        ...
        ELSIF v_item.component_type = 'deduction' THEN
          IF lower(v_item.comp_name) LIKE '%employer%' OR v_comp_code IN ('pfc', 'esic') THEN
    ...
    ELSIF v_basic_salary > 0 THEN
      -- No template: use full basic (LOP handled separately)
    ...
    -- LOP: absent days = working_days - present_days (present already includes paid leave).
    -- Unpaid leave days are implicitly LOP because they were excluded from v_present_days.
  ...
  UPDATE hr_payroll_runs SET
    ...
    status = CASE 
      WHEN status IN ('draft', 'processing', 'reviewed') THEN 'processing'
      ELSE status 
    END,
