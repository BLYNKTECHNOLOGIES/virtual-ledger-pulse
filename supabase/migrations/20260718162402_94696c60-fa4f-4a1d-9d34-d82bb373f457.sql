...
-- =====================================================================
-- F1 payroll batch: F1-3 (in-period leave days), F1-4 (LOP unpaid),
-- F1-8 (deposit dual-writer removal). Other F1 items already deployed.
-- =====================================================================

-- F1-4 data fix: mark LOP as unpaid (idempotent)
UPDATE hr_leave_types
   SET is_paid = false
 WHERE (code = 'LOP' OR lower(name) LIKE '%loss of pay%')
   AND COALESCE(is_paid, true) = true;
...
BEGIN
  ...
  -- Idempotency: reverse loan repayments recorded by prior run of this payroll
  UPDATE hr_loans l
  ...
  LOOP
    ...
    -- F1-3 FIX: in-period leave days via fn_calculate_working_days on the clipped range,
    -- capped at the request's own total_days (handles is_half_day = 0.5 correctly).
    -- F1-4: JOIN hr_leave_types and only count is_paid = true toward v_leave_days;
    --       unpaid requests accrue into v_unpaid_leave_days separately.
    ...
    -- Loans: single source of truth is trg_sync_loan_balance on hr_loan_repayments.
    v_loan_deduction := 0;
    ...
    LOOP
      ...
      IF v_deposit.is_fully_collected AND v_deposit.current_balance < v_deposit.collected_amount THEN
        ...
        END IF;
      END IF;
      -- F1-8 FIX: trg_sync_deposit_balance on hr_deposit_transactions is the single owner of
      -- collected_amount / current_balance / is_fully_collected. No manual UPDATE here.
    END LOOP;
