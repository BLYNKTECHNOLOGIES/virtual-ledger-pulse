...
BEGIN
  ...
  -- R3-1: state-machine-legal transition. draft -> processing is allowed.
  -- Re-runs on 'processing' are no-ops (OLD=NEW). Re-runs on 'reviewed' remain 'reviewed'.
  ...
  -- R3-2: restore run aggregates and processed_by attribution.
  -- R3-1: only transition 'processing' -> 'generated' (legal). Leave 'reviewed' untouched (OLD=NEW no-op).
