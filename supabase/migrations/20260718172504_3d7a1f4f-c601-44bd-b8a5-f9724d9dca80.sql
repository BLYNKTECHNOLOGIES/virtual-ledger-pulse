...
DECLARE
  ...
  v_start_status text;
  v_base_earnings numeric;      -- pre-OT, pre-arrears earnings (from template)
  v_stat_base numeric;          -- statutory base: base earnings reduced by LOP
BEGIN
  ...
  LOOP
    ...
    -- Capture pre-OT / pre-arrears earnings for statutory base
    ...
    -- Statutory base: earned regular wages (excludes OT + arrears), reduced by LOP.
    -- Used for ESI eligibility snapshot, ESI contribution wages, and Professional Tax.
    ...
    IF v_emp.uan_number IS NOT NULL OR v_emp.pf_number IS NOT NULL THEN
      ...
      -- PF Admin charges: 0.5% of EPF wages per employee. The ₹500 EPFO minimum
      -- is an establishment-wide floor applied at return-filing (ECR) time,
      -- NOT a per-employee minimum. Applying GREATEST(500, ...) per head
      -- inflated employer cost by ~₹500 × headcount every month.
    ...
    IF NOT FOUND THEN
      -- ESI eligibility snapshot must be based on regular wages ONLY.
      -- Section 2(22) of the ESI Act excludes overtime from the ₹21,000
      -- coverage test; arrears also do not enter the eligibility snapshot.
    ...
    IF v_esi_eligible AND v_emp.esi_number IS NOT NULL THEN
      -- ESI contribution wages: earned regular wages minus LOP. Overtime IS
      -- subject to ESI contribution once eligible; add it back to the base.
    ...
    -- Professional Tax slabs on earned regular wages (excl. OT + arrears, less LOP).
    -- Arrears spikes must not push an employee into a higher PT slab.
