...
-- ─── Phase 1a: v4 engine (inert — no caller wired yet) ───

-- Extend hr_attendance_daily with v4 rollup fields (additive; existing rows keep working)
ALTER TABLE public.hr_attendance_daily
  ...
  ADD COLUMN IF NOT EXISTS detected_shift_id uuid,
  ADD COLUMN IF NOT EXISTS flags jsonb NOT NULL DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS engine_version text NOT NULL DEFAULT 'legacy';

COMMENT ON COLUMN public.hr_attendance_daily.engine_version IS
  'v4 engine tag: "v4" once recomputed by hr_v4_recompute_range; "legacy" for pre-cutover or unrecomputed rows.';

-- Helper: 05:00-IST window-date of any timestamp
CREATE OR REPLACE FUNCTION public.hr_v4_window_date_of(p_ts timestamptz)
RETURNS date LANGUAGE sql STABLE SET search_path = public AS $$
  SELECT CASE
    WHEN (p_ts AT TIME ZONE 'Asia/Kolkata')::time
         < COALESCE((SELECT day_cutoff_ist FROM public.hr_attendance_engine_settings LIMIT 1), '05:00'::time)
    THEN ((p_ts AT TIME ZONE 'Asia/Kolkata')::date - 1)
    ELSE ((p_ts AT TIME ZONE 'Asia/Kolkata')::date)
  END
$$;

-- Core engine: pure, idempotent, re-runnable-from-raw over (emp, from, to).
...
BEGIN
  ...
  -- Pre-cutover: delegate to legacy rebuild (untouched span math)
  IF p_to < s_cutover_date THEN
    PERFORM public.hr_rebuild_attendance_daily_range(p_employee_id, p_from, p_to);
    RETURN;
  END IF;

  -- If range straddles the cutover, do legacy on the pre-cutover slice, v4 on the rest
  ...
  -- Expanded scan window: look back 2 days (covers night shifts + any lingering open session) and forward 1
  scan_start := ((p_from - INTERVAL '2 days')::text || ' 00:00:00 Asia/Kolkata')::timestamptz;
  scan_end   := ((p_to   + INTERVAL '1 day' )::text || ' 23:59:59 Asia/Kolkata')::timestamptz;

  -- Reset classification for punches whose window-date is in the target range
  ...
  -- Blow away existing sessions in target window-dates (will be rebuilt)
  DELETE FROM public.hr_attendance_sessions
   WHERE employee_id = p_employee_id
     AND attendance_date BETWEEN p_from AND p_to;

  -- Prime continuous state from any still-open session BEFORE scan_start
  -- (session whose IN is earlier than scan_start would have been deleted only if its window is in-range;
  --  otherwise it's preserved and we honor its open state)
  ...
  -- Chronological walk over all raw punches in the expanded window
  FOR r IN
  ...
  LOOP
    ...
    IF v_reason IS NOT NULL THEN
      -- Suppress only if this punch's window is in-range (we already reset those)
    ...
    ELSE
      -- Close session — session anchored to IN's window-date
      v_wdate := public.hr_v4_window_date_of(open_in_time);
      v_minutes := GREATEST(0, (EXTRACT(EPOCH FROM (r.punch_time - open_in_time)) / 60)::int);

      -- Only persist sessions whose window falls in the target range;
      -- sessions outside the range are participation-only (already exist untouched)
      IF v_wdate BETWEEN p_from AND p_to THEN
        ...
        INSERT INTO public.hr_attendance_sessions
              (employee_id, attendance_date, session_no,
               in_punch_id, out_punch_id, in_time, out_time, minutes,
               flags)
        VALUES (p_employee_id, v_wdate, v_next_session_no,
                open_in_id, r.id, open_in_time, r.punch_time, v_minutes,
  ...
  -- Persist any still-open session whose IN's window is in-range
  IF open_in_id IS NOT NULL THEN
    v_wdate := public.hr_v4_window_date_of(open_in_time);
    IF v_wdate BETWEEN p_from AND p_to THEN
      SELECT COALESCE(MAX(session_no), 0) + 1
      ...
      INSERT INTO public.hr_attendance_sessions
            (employee_id, attendance_date, session_no,
             in_punch_id, out_punch_id, in_time, out_time, minutes, flags)
      VALUES (p_employee_id, v_wdate, v_next_session_no,
              open_in_id, NULL, open_in_time, NULL, NULL,
              jsonb_build_object('open', true));
    END IF;
  END IF;

  -- ─── L4 rollup per window-date ───
  -- For every window-date in the range (whether or not sessions exist), upsert hr_attendance_daily + hr_attendance mirror
  RETURN QUERY
  WITH days AS (
  ...
  ins_mirror AS (
    ...
    SELECT p_employee_id, m.attendance_date, m.first_in, m.last_out,
           CASE
             ...
             ELSE 'present'
           END,
           0,     -- OT computed in Phase 2 (shift-length aware); zero-init here
           now()
      FROM merged m
      WHERE m.net_work_minutes > 0 OR m.open_count > 0     -- don't overwrite explicit leave/absent rows with a blank
    ON CONFLICT (employee_id, attendance_date) DO UPDATE
...
COMMENT ON FUNCTION public.hr_v4_recompute_range(uuid, date, date) IS
  'Attendance Engine v4 — pure idempotent L1(debounce 15s) → L2(strict alternation) → L3(sessionize) → L4(rollup) recompute over (employee, window-date range). Continuous state across midnight+cutoff. Delegates pre-cutover dates to hr_rebuild_attendance_daily_range.';

GRANT EXECUTE ON FUNCTION public.hr_v4_recompute_range(uuid, date, date) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.hr_v4_window_date_of(timestamptz) TO authenticated, service_role;
