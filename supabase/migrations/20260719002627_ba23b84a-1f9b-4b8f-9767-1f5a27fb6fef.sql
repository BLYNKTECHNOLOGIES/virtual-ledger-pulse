...
BEGIN
  ...
  -- NOTE: punch_count below reflects session-derived effective punches
  -- (closed sessions contribute 2, open sessions contribute 1). It is NOT
  -- the raw punch tally. Downstream consumers should use suppressed_count
  -- and hr_attendance_punches for raw counts.
  RETURN QUERY
  WITH days AS (
    SELECT gs::date AS d
  ...
  ins_daily AS (
    ...
    SELECT p_employee_id, m.attendance_date, m.first_in, m.last_out,
           ...
           -- A1: zero-activity days receive 'no_data' (Phase-3 will decide
           -- absent vs weekly-off vs holiday vs leave). Never write 'absent'
           -- from the recompute path while Phase 3 is disabled.
           CASE
  ...
  ins_mirror AS (
    -- A1: skip zero-activity days from the legacy mirror entirely
    -- (no ambient 'absent' rows leaking to consumers of hr_attendance).
