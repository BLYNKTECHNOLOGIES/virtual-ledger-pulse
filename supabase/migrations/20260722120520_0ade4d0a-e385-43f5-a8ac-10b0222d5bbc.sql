...
BEGIN
  ...
  -- Serialize creation for the same real-world buyer. This closes the race where
  -- two browser clicks/sessions both saw "no existing client" and inserted two
  -- pending stubs before either one wrote client_binance_usernos.
  v_lock_key := COALESCE('userno:' || v_cp_userno, 'phone:' || v_phone, 'name:' || lower(v_base_name));
  PERFORM pg_advisory_xact_lock(hashtextextended('create_buyer_client_with_evidence:' || v_lock_key, 0));

  -- 1) Binance userNo is the strongest identity anchor.
  ...
  -- 2) Phone is the client directory dedupe key. Reuse any active client with the
  -- same normalized phone; do not restrict this to PENDING rows, otherwise repeat
  -- orders from approved buyers create fresh pending stubs.
  IF v_existing_id IS NULL AND v_phone IS NOT NULL THEN
    ...
    ORDER BY CASE WHEN COALESCE(c.buyer_approval_status, 'PENDING') = 'PENDING' THEN 0 ELSE 1 END,
             c.created_at ASC
    LIMIT 1;

    IF v_existing_id IS NOT NULL AND v_cp_userno IS NOT NULL THEN
      SELECT bu.cp_userno
        INTO v_existing_userno
      FROM public.client_binance_usernos bu
      WHERE bu.client_id = v_existing_id
        AND bu.is_active = true
      LIMIT 1;

      IF v_existing_userno IS NOT NULL AND v_existing_userno <> v_cp_userno THEN
        RAISE EXCEPTION 'Phone number % is already assigned to a different Binance userNo (%). Manual review is required.', v_phone, v_existing_userno;
      END IF;
    END IF;
  END IF;

  -- 3) Last fallback: reuse a pending exact-name stub only when it has no completed
  -- sales order. This is deliberately weaker than userNo/phone and never touches
  -- approved clients.
  ...
  -- Insert the client after the serialized checks above.
