-- 1) Patch the v4 recompute: add stale-open watchdog to the IN branch.
...
BEGIN
  ...
  LOOP
    ...
    ELSIF r.punch_type = 'in' THEN
      IF open_in_id IS NOT NULL THEN
        -- A2b (NEW): stale-open watchdog on IN. If the currently-open IN is
        -- older than watchdog_hours, the prior session was never closed
        -- (device swallowed the OUT, employee forgot to punch out, etc.).
        -- Drop the stale open and accept this IN as a brand-new session,
        -- rather than incorrectly suppressing every subsequent morning IN.
        v_gap_hours := EXTRACT(EPOCH FROM (r.punch_time - open_in_time)) / 3600.0;
        IF v_gap_hours > s_watchdog_hours THEN
          open_in_id := NULL;
          open_in_time := NULL;
          -- fall through: this IN is kept as a fresh session below
        ELSE
    ...
    ELSE
      IF open_in_id IS NULL THEN
        v_reason := 'orphan_out';
      ELSE
        -- A2: watchdog guard. If an OUT arrives more than s_watchdog_hours after
        -- the open IN, do NOT close the monster session. Suppress this OUT as
        -- orphan_out and leave the open IN for Phase 4 / regularization.
        v_gap_hours := EXTRACT(EPOCH FROM (r.punch_time - open_in_time)) / 3600.0;
        IF v_gap_hours > s_watchdog_hours THEN
          v_reason := 'orphan_out';
        END IF;
  ...
  ins_daily AS (
    ...
    SELECT p_employee_id, m.attendance_date, m.first_in, m.last_out,
           ...
           CASE
             WHEN m.net_work_minutes = 0 AND m.open_count = 0 AND m.suppressed_count = 0 THEN 'no_data'
...
-- 2) Clear the two stuck open sessions blocking Priya & Shivangi.
DELETE FROM public.hr_attendance_sessions
 WHERE employee_id IN (
    '94b20609-6984-4d32-8c6a-7eac89e3c0af',  -- Priya Saxena
    '50cde3fb-e7cb-4c65-bcf1-b50605e9cb7c'   -- Shivangi Koshti
 )
   AND out_punch_id IS NULL;

-- 3) Un-suppress their post-cutover punches so the patched engine can reclassify.
UPDATE public.hr_attendance_punches
   SET effective = true, suppressed_reason = NULL
 WHERE employee_id IN (
    '94b20609-6984-4d32-8c6a-7eac89e3c0af',
    '50cde3fb-e7cb-4c65-bcf1-b50605e9cb7c'
 )
   AND punch_time >= '2026-07-17 00:00:00+05:30'::timestamptz;

-- 4) Recompute their attendance for the affected range.
DO $$
DECLARE
  emp uuid;
BEGIN
  FOREACH emp IN ARRAY ARRAY[
    '94b20609-6984-4d32-8c6a-7eac89e3c0af'::uuid,
    '50cde3fb-e7cb-4c65-bcf1-b50605e9cb7c'::uuid
  ] LOOP
    PERFORM public.hr_v4_recompute_range(emp, '2026-07-17'::date, CURRENT_DATE);
  END LOOP;
END $$;