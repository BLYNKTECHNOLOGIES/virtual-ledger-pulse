...
BEGIN
  ...
  att AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE r.has_evidence)::numeric AS evidence_days
    FROM att_rows r
    GROUP BY r.emp_id
  ),
  lv AS (
  ...
  calc AS (
    SELECT
      ...
      COALESCE(att.evidence_days,0)::numeric AS evidence_days,
  ...
  SELECT
    ...
    CASE
      ...
      -- No biometric signal at all this month: cannot be treated as absence.
      WHEN c.evidence_days = 0 AND c.paid_d = 0 THEN 0::numeric
    ...
    CASE
      ...
      WHEN c.evidence_days = 0 AND c.paid_d = 0
    ...
    c.wo_source,
    CASE
      WHEN c.evidence_days = 0 AND NOT c.is_contractor
