...
BEGIN
  ...
  SELECT r.employee_id, r.working_days, r.maintained_present, r.maintained_half,
    r.maintained_absent, r.paid_leave_days, r.unpaid_leave_days,
    0::numeric AS held_harmless_days,
    GREATEST(0, r.working_days-r.maintained_present-(r.maintained_half*0.5)-r.paid_leave_days-r.unpaid_leave_days-r.maintained_absent),
    CASE WHEN r.is_contractor OR r.maintained_days=0 THEN 0::numeric
      ELSE GREATEST(0, LEAST(r.working_days,
        r.working_days-r.maintained_present-(r.maintained_half*0.5)-r.paid_leave_days))::numeric END,
    ...
    CASE WHEN r.is_contractor THEN 'LOP = 0 (contract employee — Attendance Summary shown for reference)'
      ...
      ELSE format('LOP = working days %s − (Attendance Summary present %s + half-day credit %s + paid leave %s) = %s',
        r.working_days,r.maintained_present,r.maintained_half*0.5,r.paid_leave_days,
        GREATEST(0,LEAST(r.working_days,r.working_days-r.maintained_present-(r.maintained_half*0.5)-r.paid_leave_days))) END,
    CASE WHEN r.maintained_days=0 AND NOT r.is_contractor
