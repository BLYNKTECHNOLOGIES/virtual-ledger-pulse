...
BEGIN
  ...
  IF v_count = 0 THEN
    ...
    VALUES
      (p_employee_id, v_basic,   ROUND(p_new_total * 0.50, 2), false, true, 'legacy_local', CURRENT_DATE),
      (p_employee_id, v_hra,     ROUND(p_new_total * 0.25, 2), false, true, 'legacy_local', CURRENT_DATE),
      (p_employee_id, v_special, ROUND(p_new_total * 0.15, 2), false, true, 'legacy_local', CURRENT_DATE),
      (p_employee_id, v_lta,     p_new_total
         - ROUND(p_new_total * 0.50, 2) - ROUND(p_new_total * 0.25, 2) - ROUND(p_new_total * 0.15, 2),
       false, true, 'legacy_local', CURRENT_DATE);
    RETURN;
  END IF;

  IF v_old_total = 0 THEN
    UPDATE public.hr_employee_salary_structures s
       SET amount = p_new_total, updated_at = now()
     WHERE s.id = (
  ...
  UPDATE public.hr_employee_salary_structures
     SET amount = ROUND(amount * v_ratio, 2), updated_at = now()
   WHERE employee_id = p_employee_id AND is_active = true;
  ...
  UPDATE public.hr_employee_salary_structures s
     SET amount = amount + (p_new_total - (SELECT s FROM sums)), updated_at = now()
