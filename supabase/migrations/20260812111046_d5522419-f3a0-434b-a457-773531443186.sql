...
BEGIN
      ...
      IF v_wdate BETWEEN p_from AND p_to
        ...
        INSERT INTO public.hr_attendance_sessions
              (employee_id, attendance_date, session_no, in_punch_id, out_punch_id,
               in_time, out_time, duration_minutes, source)
        VALUES (p_employee_id, v_wdate, v_next_session_no, open_in_id, r.id,
                open_in_time, r.punch_time, v_minutes, 'engine');
      END IF;
      open_in_id := NULL;
      open_in_time := NULL;
    END IF;
  ...
  agg_sess AS (
    SELECT attendance_date,
           COALESCE(SUM(duration_minutes), 0)::int AS net_work_minutes,
  ...
  ins_daily AS (
    ...
    ON CONFLICT (employee_id, attendance_date) DO UPDATE
       SET first_in = EXCLUDED.first_in, last_out = EXCLUDED.last_out,
           ...
           -- ABSENT-PRESERVING: a rebuild with zero punches must never erase a
           -- judgement already made by the absent-marker / HR (absent, leave,
           -- week-off, holiday). Only overwrite when the rebuild has real data.
           status = CASE
