...
BEGIN
  ...
  -- Clear every single-column FK in the public schema that points at either
  -- public.users OR auth.users. Auth-side references must be cleared too,
  -- otherwise auth.admin.deleteUser() fails and the login stays active.
  FOR _ref IN
    SELECT DISTINCT
  ...
  RETURN jsonb_build_object(
    'success', true,
    'mode', 'catalog_driven_cleanup_v4_auth_aware',
