...
BEGIN
  ...
  -- DAY-SCOPED LOP: window the engine to this single date (previously the
  -- month-scoped helper was called, which surfaced month-to-date LOP on a day tile).
  BEGIN
    SELECT lop_days INTO v_lop_row
    FROM public.hr_lop_days_window(ARRAY[p_employee_id]::uuid[], p_date, p_date, p_date);
