...
BEGIN
  ...
  LOOP
    ...
    -- An approved regularization is authoritative for its window: raw device
    -- punches falling strictly inside it are set aside so the corrected
    -- in/out pair drives worked minutes, status and late/early stats.
       ...
       AND EXISTS (
         SELECT 1 FROM public.hr_attendance_regularization_requests g
          WHERE g.employee_id = p_employee_id
            ...
            AND r.punch_time > g.requested_check_in
            AND r.punch_time < g.requested_check_out
       )
    THEN
