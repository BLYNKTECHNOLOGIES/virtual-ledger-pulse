-- =====================================================================
-- Policy-based LOP integration
-- Makes hr_attendance_policies.late_count_for_lop and
-- half_day_count_for_lop actually drive LOP calculation.
-- Rule: 0 (or NULL) means the corresponding policy is disabled.
-- =====================================================================

-- -----------------------------------------------------------------
-- 1. Canonical month-level LOP: add policy-based late/half-day LOP
-- -----------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    ...
    format('LOP = WD %s − (present %s + paid_leave %s + incomplete_held %s) + policy_lop(%s) = %s',
...
'Canonical month-level LOP. Adds policy-based LOP from hr_attendance_policies: late_count_for_lop and half_day_count_for_lop. A threshold of 0 disables that policy.';

-- -----------------------------------------------------------------
-- 2. Windowed LOP: add policy-based late/half-day LOP inside window
-- -----------------------------------------------------------------
...
RETURNS TABLE(
  employee_id uuid, working_days numeric, present_days numeric, paid_leave_days numeric,
  unpaid_leave_days numeric, incomplete_held_days numeric, absent_days numeric, half_days numeric,
  lop_days numeric, formula text, weekly_off_days integer[], weekly_off_source text, config_errors text[]
)
...
BEGIN
  ...
  lv_days AS (
    SELECT lv.emp_id, lv.is_paid,
           ROUND(SUM((CASE WHEN lv.is_half_day THEN 0.5 ELSE 1 END) * lv.paid_ratio)::numeric, 2) AS paid_days_in_win,
           ROUND(SUM((CASE WHEN lv.is_half_day THEN 0.5 ELSE 1 END) * (1 - lv.paid_ratio))::numeric, 2) AS unpaid_days_in_win
    FROM lv
  ...
  SELECT c.emp_id, c.wdays, c.present_d, c.paid_d, c.unpaid_d, c.held_d, c.absent_d, c.half_d,
    ...
    CASE WHEN c.is_contractor
         ...
         ELSE format('LOP = elapsed WD %s (of %s) − (verified present %s + paid_leave %s + held_harmless %s) + policy_lop(%s) = %s%s',
...
COMMENT ON FUNCTION public.hr_lop_days_window(uuid[], date, date, date) IS
'Windowed LOP helper. Adds policy-based LOP from hr_attendance_policies using late/half-day counts inside the requested window. Threshold 0 disables the policy.';
