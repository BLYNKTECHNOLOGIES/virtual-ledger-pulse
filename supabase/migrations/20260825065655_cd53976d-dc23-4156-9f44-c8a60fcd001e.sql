...
BEGIN
  ...
  lv AS (
    -- paid_ratio: honours the HR cascade. If balances could only cover part of
    -- the request, only that part counts as paid leave; the rest falls to LOP.
  ...
  lv_days AS (
    SELECT lv.emp_id, lv.is_paid,
           ROUND(SUM((CASE WHEN lv.is_half_day THEN 0.5 ELSE 1 END) * lv.paid_ratio)::numeric, 2) AS paid_days_in_month,
           ROUND(SUM((CASE WHEN lv.is_half_day THEN 0.5 ELSE 1 END) * (1 - lv.paid_ratio))::numeric, 2) AS unpaid_days_in_month
    FROM lv
    ...
    -- a day actually worked on approved leave is credited as attendance and the
    -- balance is restored — never count it as leave as well.
