-- Make an approved regularization authoritative for the WHOLE attendance window,
-- not just for punches falling strictly inside the requested in/out pair.
-- Previously a raw device punch slightly BEFORE the approved in-time (or after the
-- approved out-time) survived and became first_in / last_out, so late / early-out /
-- overtime were computed from the un-regularized clock.
...
BEGIN
  ...
  LOOP
    ...
    -- An approved regularization with a complete in/out pair is authoritative for
    -- its entire attendance window: every raw device punch belonging to that window
    -- is set aside so the approved times drive worked minutes, status, late,
    -- early-out and overtime.
    IF COALESCE(r.device_name, '') <> 'REGULARIZATION'
       AND EXISTS (
         SELECT 1 FROM public.hr_attendance_regularization_requests g
          WHERE g.employee_id = p_employee_id
            ...
            AND (
              g.attendance_date = public.hr_v4_window_date_of(r.punch_time)
              OR (r.punch_time > g.requested_check_in AND r.punch_time < g.requested_check_out)
            )
       )
    THEN
