CREATE OR REPLACE FUNCTION public.compliance_payload_attachments(p_payload jsonb, p_source text)
RETURNS text[]
LANGUAGE sql
IMMUTABLE
SET search_path TO 'public'
AS $$
  SELECT COALESCE(array_agg(DISTINCT x) FILTER (WHERE x IS NOT NULL AND x <> ''), '{}'::text[])
  FROM (
    SELECT jsonb_array_elements_text(p_payload->'documents_attached') AS x
    WHERE p_source = 'bank_cases' AND jsonb_typeof(p_payload->'documents_attached') = 'array'
    UNION ALL
    SELECT p_payload->>'supporting_document' WHERE p_source = 'bank_cases'
    UNION ALL
    SELECT jsonb_array_elements_text(p_payload->'attachment_urls')
    WHERE p_source = 'compliance_case_updates' AND jsonb_typeof(p_payload->'attachment_urls') = 'array'
    UNION ALL
    SELECT p_payload->>'file_url' WHERE p_source = 'compliance_documents'
  ) t;
$$;

REVOKE ALL ON FUNCTION public.compliance_payload_attachments(jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.compliance_payload_attachments(jsonb, text) TO authenticated, service_role;
...
audit_events AS (
  SELECT a.at, a.table_name AS source, a.action, a.record_id, a.changed_by,
         ...
         public.compliance_payload_attachments(a.payload, a.table_name) AS attachments
  FROM audit a
),
seed AS (
  SELECT c.created_at AS at, 'bank_cases' AS source, 'INSERT' AS action, c.id AS record_id, c.created_by AS changed_by,
         ...
         COALESCE(c.documents_attached,'{}'::text[]) || COALESCE(ARRAY[c.supporting_document]::text[],'{}'::text[]) AS attachments
  ...
  SELECT i.created_at, 'account_investigations', 'INSERT', i.id, NULL,
         ...
         NULLIF(concat_ws(' · ', i.status, i.priority),''), '{}'::text[], '{}'::text[]
  ...
  SELECT d.created_at, 'compliance_documents', 'INSERT', d.id, d.uploaded_by,
         ...
         COALESCE(ARRAY[d.file_url]::text[],'{}'::text[])
  ...
  SELECT bc.created_at, 'bank_communications', 'INSERT', bc.id, NULL,
         ...
         NULLIF(concat_ws(' · ', bc.mode, bc.contact_person),''), '{}'::text[], '{}'::text[]
  ...
  SELECT cu.created_at, 'compliance_case_updates', 'INSERT', cu.id, cu.created_by,
         ...
         COALESCE(cu.attachment_urls,'{}'::text[])
...
combined AS (
  ...
  UNION ALL
  SELECT at, source, action, record_id, changed_by, title, subtitle, fields, attachments FROM seed_events
),
filtered AS (
...
SELECT jsonb_build_object(
  ...
  'events', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      ...
      'attachments', to_jsonb(COALESCE(array_remove(attachments, NULL),'{}'::text[]))
