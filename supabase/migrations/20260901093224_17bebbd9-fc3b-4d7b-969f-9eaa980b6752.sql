...
BEGIN
  ...
  FOR r IN
    SELECT id, punch_time, punch_type, device_name
  ...
  LOOP
    ...
    IF public.hr_v4_regularization_override(p_employee_id, r.punch_time, r.device_name)
    THEN
      v_reason := 'regularization_override';
    ELSIF last_kept_ts IS NOT NULL
    ...
    ELSIF r.punch_type = 'in' THEN
      IF open_in_id IS NOT NULL THEN
        ...
        IF v_gap_hours > s_watchdog_hours THEN
          -- The earlier IN is stale (forgotten punch-out). Persist it as an OPEN
          -- session so its day keeps its punch evidence and is honestly reported
          -- as "incomplete" instead of silently losing the punch.
  ...
  ins_daily AS (
    ...
    SELECT p_employee_id, m.attendance_date, m.first_in, m.last_out,
           ...
           CASE
             ...
             WHEN m.net_work_minutes < public.hr_v4_half_day_minutes() THEN 'absent'
             WHEN m.net_work_minutes < public.hr_v4_full_day_minutes() THEN 'half_day'
             ELSE 'present'
           END,
    ...
    ON CONFLICT (employee_id, attendance_date) DO UPDATE
       SET first_in = EXCLUDED.first_in, last_out = EXCLUDED.last_out,
           ...
           status = CASE
                      -- Only statuses the engine itself never derives (leave,
                      -- holiday, week off, HR-marked days) survive a no-evidence
                      -- recompute. Engine-derived statuses such as 'incomplete'
                      -- MUST NOT linger once their punch evidence is gone —
                      -- that is what made HRMS and the ERP profile disagree.
                      WHEN EXCLUDED.status = 'no_data'
                       AND d.status IS NOT NULL
                       AND d.status NOT IN ('no_data','incomplete','in_progress','present','half_day','absent')
                        THEN d.status
  ...
  del_mirror AS (
    -- Legacy mirror rows left behind as 'incomplete' after their evidence
    -- disappeared would keep feeding stale statuses to legacy readers.
    DELETE FROM public.hr_attendance h
     USING merged m
     WHERE h.employee_id = p_employee_id
       AND h.attendance_date = m.attendance_date
       AND m.net_work_minutes = 0 AND m.open_count = 0
       AND h.attendance_status = 'incomplete'
    RETURNING 1
  )
  ...
  resolved AS (
    SELECT t.attendance_date, t.first_in, t.last_out,
      ...
      FROM tgt t
      LEFT JOIN LATERAL public.hr_v4_detect_shift(p_employee_id, t.attendance_date, t.first_in, t.last_out) det ON true
  ),
  metrics AS (
  ...
  UPDATE public.hr_attendance_daily d
     SET detected_shift_id = mx.shift_id,
         ...
         flags = COALESCE(d.flags, '{}'::jsonb) || jsonb_build_object(
                   ...
                   'grace_minutes', mx.grace_minutes,
                   'is_overnight_shift', mx.is_overnight,
                   'ot_minutes', LEAST(COALESCE(mx.ot_minutes, 0), (s_ot_daily * 60)::int)
                 ),
         updated_at = now()
    FROM metrics mx
   WHERE d.employee_id = p_employee_id
  ...
  PERFORM public.hr_v4_rebuild_late_early_register(p_employee_id, p_from, p_to);
END;
$function$;