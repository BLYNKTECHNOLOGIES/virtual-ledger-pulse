...
-- 1. Plan configuration columns
ALTER TABLE public.hr_leave_accrual_plans
  ADD COLUMN IF NOT EXISTS accrual_day integer NOT NULL DEFAULT 10,
  ADD COLUMN IF NOT EXISTS start_trigger text NOT NULL DEFAULT 'joining',
  ADD COLUMN IF NOT EXISTS cycle_basis text NOT NULL DEFAULT 'calendar';

DO $$ BEGIN
  ALTER TABLE public.hr_leave_accrual_plans
    ADD CONSTRAINT chk_accrual_day CHECK (accrual_day BETWEEN 1 AND 28);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.hr_leave_accrual_plans
    ADD CONSTRAINT chk_start_trigger CHECK (start_trigger IN ('joining','probation_end'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.hr_leave_accrual_plans
    ADD CONSTRAINT chk_cycle_basis CHECK (cycle_basis IN ('calendar','anniversary'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DECLARE
  ...
  v_exists int;
BEGIN
  -- single-flight: a manual run and the cron run cannot credit twice
  ...
  LOOP
    ...
    -- credit-day gate (all cycles): nothing is credited before the configured day
    ...
    LOOP
      ...
      IF v_plan.effective_from > p_accrual_date THEN CONTINUE; END IF;

      -- sick leave is never credited while on probation
      IF v_is_sl AND public.hr_is_on_probation(v_emp.employee_id, p_accrual_date) THEN
        CONTINUE;
      END IF;

      -- due? (per employee, never plan-wide)
      ...
      v_cap := v_plan.max_accrual; -- NULL = unlimited

      -- year-boundary carry forward: seed this year's bucket from last year's remainder
...
-- 3. Schedule: one daily run at 10:00 IST (04:30 UTC)
DO $$ BEGIN
  PERFORM cron.unschedule('monthly-leave-accrual');
EXCEPTION WHEN OTHERS THEN NULL; END $$;

DO $$ BEGIN
  PERFORM cron.unschedule('daily-leave-accrual');
EXCEPTION WHEN OTHERS THEN NULL; END $$;

SELECT cron.schedule('daily-leave-accrual', '30 4 * * *', $$SELECT public.run_leave_accrual()$$);
