-- 1) Feb-29 safety for recurring holidays inside the windowed engine
...
BEGIN
  ...
  hols AS (
    ...
    WHERE h.is_active = true AND h.recurring = true
      ...
      -- guard: a recurring 29-Feb holiday must not blow up in a non-leap year
  ...
  blackout AS (
    -- tenant-wide device/site outage: nobody in the company punched that day
    SELECT d::date AS dt
...
-- 2) hr_lop_days becomes a thin, correct delegate (single source of truth)
CREATE OR REPLACE FUNCTION public.hr_lop_days(p_employee_ids uuid[], p_period_month date)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT * FROM public.hr_lop_days_window(
    p_employee_ids,
    p_period_month,
