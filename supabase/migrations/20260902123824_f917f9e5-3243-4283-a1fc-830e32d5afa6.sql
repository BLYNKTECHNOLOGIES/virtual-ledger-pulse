...
BEGIN
  ...
  -- LOP rows are identified by their source (stable) and, for hand-staged rows,
  -- by the wording operators actually use. The label carries the day count and
  -- must never be the identity key.
  s4 AS (
  ...
  -- A month with genuinely zero computed LOP is legitimately finished.
  s4z AS (
  ...
  -- Recovery vocabulary as it actually exists: scheduled -> pushed -> paid.
  -- 'skipped'/'cancelled' rows are out of scope and must not block the step.
  s5r AS (
  ...
  -- Only a usable (computed) shadow run counts as a comparison.
  s8 AS (
  ...
  -- Drift belongs to the month it existed in, not to whichever night it was
  -- last re-scanned. Anything first seen by month end and still open blocks.
  s9 AS (
