-- 1) Owner decision: no extra penalty for half-days; only the actual half day is lost.
UPDATE public.hr_attendance_policies
   SET half_day_count_for_lop = 0
 WHERE is_active = true AND is_default = true;

-- 2) LOP engine: single-credit regularised days, and no double credit when a day is
--    both fully attended and covered by approved leave.
...
BEGIN
  ...
  -- days already credited in full by attendance: an overlapping approved leave on the
  -- same date must not be credited a second time (half-days deliberately excluded —
  -- half work + half leave is a legitimate combination)
  full_credit_days AS (
    SELECT r.emp_id, r.dt FROM day_rows r
    WHERE r.st = 'present' AND (r.has_evidence OR r.regularized)
  ),
  att AS (
    SELECT r.emp_id,
      ...
      -- regularisation no longer excuses a day here: an approved regularisation already
      -- makes the day count as attended above (owner rule: follow the day's final status)
  ...
  SELECT c.emp_id, c.wdays, c.present_d, c.paid_d, c.unpaid_d, c.held_d, c.absent_d, c.half_d,
    ...
    CASE WHEN c.is_contractor
         ...
         ELSE format('LOP = elapsed WD %s (of %s) − (verified present %s + paid_leave %s + held_harmless %s) + policy_lop(%s) = %s%s%s',
...
-- 3) Comp-off pool: consume comp-off only when comp-off actually funded the leave.
...
AS $function$
  ...
  taken AS (
    ...
    FROM (
      -- comp-off used as an explicit fallback on any leave request
      SELECT r.employee_id, c.days AS d
      FROM cons c JOIN reqs r ON r.id = c.request_id
      WHERE c.source = 'compoff_fallback'
      UNION ALL
      -- comp-off-type requests: only the portion actually funded from comp-off.
      -- Requests with no consumption breakdown fall back to their full length.
