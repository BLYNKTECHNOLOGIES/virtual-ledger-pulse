...
BEGIN
  ...
  -- Days the attendance engine actually marked as worked (same predicate the
  -- comp-off grant trigger uses), keyed on the engine's own window date.
  qualifying AS (
  ...
  -- Raw punch/session evidence, mapped to the engine window date (day cutoff),
  -- so a 01:00 punch belongs to the previous working day, not to a "holiday".
  evidence AS (
  ...
  credits AS (
    SELECT c.employee_id AS emp_id,
           SUM(c.credit_days)::numeric AS days,
           jsonb_agg(jsonb_build_object(
             'date', c.credit_date, 'type', c.credit_type, 'days', c.credit_days,
             'notes', c.notes,
             'duplicate', (COUNT(*) OVER (PARTITION BY c.employee_id, c.credit_date) > 1)
           ) ORDER BY c.credit_date) AS detail
