...
BEGIN
  ...
  -- ------- leave balance ledger (reporting only) -------
  -- Used in the month, per category, from the same clipped consumption above.
  used_cat AS (
    SELECT c.emp_id,
  ...
  -- Credited in the month by the accrual engine (CL / SL plans).
  accrued AS (
  ...
  -- Cumulative live balance for CL / SL, same rule the Leave Allocations page
  -- uses: sum of non-expired allocations up to and including this month.
  alloc AS (
  ...
  -- Comp-off pool for the month (opening / earned / taken / available).
  co_pool AS (
  ...
  SELECT em.emp_id,
         ...
         jsonb_build_object(
           'cl', jsonb_build_object(
             ...
             'closing',  ROUND(COALESCE(al.cl_bal,0),2),
             'opening',  ROUND(COALESCE(al.cl_bal,0) + COALESCE(uc.cl_used,0) - COALESCE(ac.cl_cr,0),2)
           ),
           'sl', jsonb_build_object(
             'credited', ROUND(COALESCE(ac.sl_cr,0),2),
             'used',     ROUND(COALESCE(uc.sl_used,0),2),
             'closing',  ROUND(COALESCE(al.sl_bal,0),2),
             'opening',  ROUND(COALESCE(al.sl_bal,0) + COALESCE(uc.sl_used,0) - COALESCE(ac.sl_cr,0),2)
           ),
...
REVOKE ALL ON FUNCTION public.hr_leave_month_breakdown(uuid[], date) FROM PUBLIC, anon;
