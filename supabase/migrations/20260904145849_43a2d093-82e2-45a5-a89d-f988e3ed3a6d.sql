...
BEGIN
  ...
  -- Owner rule (2026-09-04): a day whose marked status is 'present' is credited as
  -- attended even when the biometric device holds no punch for it (pre-enrolment
  -- windows for new joiners). Evidence is still tracked, but only for reporting.
  full_credit_days AS (
  ...
  att AS (
    SELECT r.emp_id,
      ...
      -- retained for transparency only: marked present/half-day without any punch
...
CREATE OR REPLACE VIEW public.hr_attendance_day_v AS
 SELECT employee_id,
    attendance_date AS date,
        CASE
            WHEN hr_is_holiday(attendance_date) AND (COALESCE(status, 'no_data'::text) = ANY (ARRAY['no_data'::text, 'absent'::text, 'no_punch'::text])) AND first_in IS NULL THEN 'holiday'::text
    ...
    engine_version,
        CASE
            WHEN hr_is_holiday(attendance_date) AND first_in IS NULL THEN 0::numeric
    ...
    (COALESCE(total_hours, 0::numeric) > 0
      OR first_in IS NOT NULL
      OR COALESCE(punch_count, 0) > 0
      OR COALESCE(session_count, 0) > 0) AS evidence_backed
