    ...
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE lower(COALESCE(x.status,'')) NOT IN ('pending','scheduled','draft')
          AND (
            (x.is_one_time AND COALESCE(x.payout_channel, '') <> 'outside_payroll' AND (x.razorpay_pushed_at IS NULL OR x.razorpay_push_error IS NOT NULL))
      ...
      COALESCE(SUM(x.one_time_amount) FILTER (WHERE x.one_time_amount IS NOT NULL AND COALESCE(x.payout_channel, '') <> 'outside_payroll'), 0)::numeric AS one_time_total,
