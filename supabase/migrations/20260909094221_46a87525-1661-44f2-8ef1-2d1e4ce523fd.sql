...
AS $function$
  ...
  select
    ...
    coalesce(sum(nullif(s.total_price,'')::numeric) filter (where s.order_status = 'COMPLETED'), 0),
    coalesce(avg(nullif(s.total_price,'')::numeric) filter (where s.order_status = 'COMPLETED'), 0),
    coalesce(percentile_cont(0.5) within group (order by nullif(s.total_price,'')::numeric)
               filter (where s.order_status = 'COMPLETED'), 0),
    coalesce(sum(nullif(s.amount,'')::numeric) filter (where s.order_status = 'COMPLETED'), 0),
    min(s.create_time),
    max(s.create_time),
    (select pay_method_name from paym),
    (select round(avg(pay_min)::numeric, 1) from timings where pay_min is not null),
    (select count(*)::bigint from timings where pay_min is not null),
    (select round(avg(rel_min)::numeric, 1) from timings where rel_min is not null),
