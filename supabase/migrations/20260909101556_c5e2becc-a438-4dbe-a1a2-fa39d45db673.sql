...
AS $function$
  ...
  SELECT
    ...
    coalesce(nullif(h.amount::text, ''), c.raw->>'amount', '0') AS amount,
    coalesce(nullif(h.total_price::text, ''), c.raw->>'totalPrice', '0') AS total_price,
    -- Prefer whichever source is fresher: the live active-order cache refreshes
    -- every few seconds, while binance_order_history syncs every few minutes.
    coalesce(
      CASE
        WHEN c.order_number IS NOT NULL
         AND (h.synced_at IS NULL OR c.updated_at > h.synced_at)
        THEN CASE c.raw->>'orderStatus'
               WHEN '1' THEN 'TRADING'
