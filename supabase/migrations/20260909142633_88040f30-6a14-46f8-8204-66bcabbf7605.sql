CREATE INDEX IF NOT EXISTS idx_cp_order_identity_acct_userno
  ON public.cp_order_identity (exchange_account_id, cp_userno)
  WHERE cp_userno IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_cp_order_identity_acct_nickname
  ON public.cp_order_identity (exchange_account_id, nickname)
...
AS $function$
  ...
  direct_fallback AS (
    SELECT
      ...
      COALESCE(
        CASE
          WHEN h.counter_part_nick_name IS NOT NULL
            AND h.counter_part_nick_name NOT LIKE '%*%'
          THEN h.counter_part_nick_name
        END,
        (
          SELECT v.nickname
          FROM (VALUES
            (h.order_detail_raw->>'buyerNickname'),
            (h.order_detail_raw->>'sellerNickname')
          ) AS v(nickname)
          WHERE v.nickname IS NOT NULL
            AND NOT EXISTS (
              SELECT 1 FROM public.cp_self_nicknames s WHERE s.nick = v.nickname
            )
          LIMIT 1
        )
      ) AS nickname,
  ...
  current_identity AS (
    ...
    UNION ALL
    SELECT * FROM direct_fallback
    WHERE NOT EXISTS (SELECT 1 FROM identity_source)
    LIMIT 1
  ),
  matched_orders AS MATERIALIZED (
    ...
    WHERE EXISTS (SELECT 1 FROM current_identity)
  ),
  ...
  timings AS (
    SELECT
      CASE
        WHEN order_detail_raw->>'notifyPayTime' ~ '^[0-9]+$'
      ...
      END AS pay_min,
      CASE
        WHEN order_detail_raw->>'confirmPayTime' ~ '^[0-9]+$'
  ...
  profile AS (
    SELECT jsonb_build_object(
      ...
      'first_trade_time', min(s.create_time),
      'last_trade_time', max(s.create_time),
      'top_pay_method', (
        SELECT x.pay_method_name FROM scoped x
        WHERE x.pay_method_name IS NOT NULL AND x.pay_method_name <> ''
        GROUP BY x.pay_method_name ORDER BY count(*) DESC LIMIT 1
      ),
  ...
  past_orders AS (
    SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      'fiat_unit', x.fiat_unit,
      'create_time', x.create_time,
      'exchange_account_id', x.exchange_account_id,
    ...
    FROM (
      ...
      FROM scoped s
      WHERE s.order_number <> p_order_number
  ...
  SELECT jsonb_build_object(
    'profile', CASE
      ...
      ELSE (SELECT value FROM profile)
    END,
