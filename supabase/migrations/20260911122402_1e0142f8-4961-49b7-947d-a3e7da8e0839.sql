...
BEGIN
  -- Split-payment orders keep one settlement row per gateway split, written
  -- explicitly by the sales screens. Never touch them here.
  IF COALESCE(NEW.is_split_payment, false) THEN
    RETURN NEW;
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND OLD.sales_payment_method_id IS NOT NULL
    ...
    IF v_old_is_gateway THEN
      ...
      WHERE sales_order_id = NEW.id
        AND payment_method_id = OLD.sales_payment_method_id
        AND status <> 'SETTLED';
    END IF;
  END IF;
