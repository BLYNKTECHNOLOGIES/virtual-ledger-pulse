CREATE OR REPLACE FUNCTION public.upsert_terminal_appeal_case(p_order_number text, p_source text DEFAULT 'manual_request'::text, p_status text DEFAULT 'requested'::text, p_request_reason text DEFAULT NULL::text, p_requested_from_case_id uuid DEFAULT NULL::uuid, p_adv_no text DEFAULT NULL::text, p_trade_type text DEFAULT NULL::text, p_asset text DEFAULT NULL::text, p_fiat_unit text DEFAULT 'INR'::text, p_total_price numeric DEFAULT NULL::numeric, p_counterparty_nickname text DEFAULT NULL::text, p_binance_status text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT public.terminal_order_has_active_appeal_evidence(p_order_number) INTO v_has_active_complaint;
  ...
  ELSIF public.terminal_order_is_final_status(v_authoritative_status) AND p_source = 'binance_status' AND p_status = 'under_appeal' AND NOT v_has_active_complaint THEN
  ...
  IF v_existing_status IS DISTINCT FROM v_effective_status THEN
    INSERT INTO public.terminal_appeal_case_events (case_id, order_number, event_type, actor_user_id, note, previous_value, new_value)
    VALUES (v_case_id, p_order_number, 'status_changed', v_actor, p_request_reason, jsonb_build_object('from_status', v_existing_status), jsonb_build_object('to_status', v_effective_status, 'binance_status', v_effective_binance_status));
  END IF;
