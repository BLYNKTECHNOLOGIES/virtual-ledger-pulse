...
BEGIN
  ...
  LOOP
    -- A sale can have MULTIPLE payment legs of the SAME amount (e.g. 3000 + 3000 on two POS
    -- machines). Compare how many legs of this amount were already settled against how many
    -- legs of this amount actually exist for the sale. Only a genuine surplus is a duplicate.
    ...
    WHERE sps.sales_order_id = v_pending_record.sales_order_id
      AND abs(sps.amount - v_pending_record.total_amount) < 0.01;

    -- No split rows (single-payment sale): a single settled entry of this amount is a duplicate.
    IF v_leg_count = 0 THEN
      v_leg_count := 1;
    END IF;

    IF v_settled_count >= v_leg_count THEN
      -- Never delete the row: keep it visible/auditable and abort the batch instead.
  ...
  IF v_skipped_count > 0 THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', v_skipped_count || ' selected transaction(s) appear to be already settled in another batch. Nothing was settled. Please refresh and review the selection.',
  ...
  IF p_mdr_amount > 0 THEN
    INSERT INTO bank_transactions (
      ...
      CURRENT_DATE, 'Bank Charges', v_settlement_batch_id || '-MDR', p_created_by
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'transaction_count', array_length(v_valid_ids, 1)
  );
END;
$function$;