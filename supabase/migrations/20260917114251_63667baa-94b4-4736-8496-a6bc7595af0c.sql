...
-- Drop historical rows for accounts that are no longer tracked (ASEC).
DELETE FROM public.terminal_ad_uptime_shift_summary s
WHERE s.exchange_account_id <> '00000000-0000-0000-0000-000000000000'::uuid
  AND NOT EXISTS (
    SELECT 1 FROM public.terminal_exchange_accounts ea
    WHERE ea.id = s.exchange_account_id AND ea.ad_uptime_tracked
  );

DELETE FROM public.terminal_ad_uptime_monthly_summary m
WHERE m.exchange_account_id <> '00000000-0000-0000-0000-000000000000'::uuid
  AND NOT EXISTS (
    SELECT 1 FROM public.terminal_exchange_accounts ea
    WHERE ea.id = m.exchange_account_id AND ea.ad_uptime_tracked
  );