CREATE OR REPLACE FUNCTION public.cbt_create_question(p_type cbt_question_type, p_category_tag text, p_difficulty cbt_difficulty, p_prompt text, p_marks numeric DEFAULT 1, p_options jsonb DEFAULT NULL::jsonb, p_correct_option_id text DEFAULT NULL::text, p_numeric_answer numeric DEFAULT NULL::numeric, p_numeric_tolerance numeric DEFAULT 0, p_rubric jsonb DEFAULT NULL::jsonb, p_role_codes text[] DEFAULT NULL::text[], p_explanation text DEFAULT NULL::text, p_approve boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF p_type NOT IN ('mcq','numeric','written','typing_passage') THEN
  ...
  ELSIF p_type = 'numeric' THEN
    ...
    END IF;
  ELSIF p_type = 'typing_passage' THEN
    v_prompt := btrim(regexp_replace(v_prompt, '\s+', ' ', 'g'));
    v_words := array_length(regexp_split_to_array(v_prompt, ' '), 1);
    IF COALESCE(v_words, 0) < 40 THEN
      RAISE EXCEPTION 'A typing passage needs at least 40 words';
    END IF;
  END IF;
  ...
  VALUES (v_q, 1, jsonb_strip_nulls(jsonb_build_object(
    'prompt', CASE WHEN p_type = 'typing_passage' THEN NULL ELSE v_prompt END,
    'passage', CASE WHEN p_type = 'typing_passage' THEN v_prompt ELSE NULL END,
    'word_count', CASE WHEN p_type = 'typing_passage' THEN v_words ELSE NULL END,
    'marks', p_marks,
