...
BEGIN
  ...
  ELSIF s.section_type = 'memory_recall' THEN
    ...
    LOOP
      ...
      -- The key is one character per element; normalise the typed answer the same way
      -- (strip everything non-alphanumeric, then split into single characters) so that
      -- "4K9T", "4 K 9 T" and "4k9t" are all equivalent, matching the UI hint.
      ansq := regexp_split_to_array(
                regexp_replace(COALESCE(rec.response->>'value', ''), '[^0-9A-Za-z]', '', 'g'), '');
