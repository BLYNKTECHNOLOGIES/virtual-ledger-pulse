REVOKE EXECUTE ON FUNCTION public.sync_sales_order_settlement_status(uuid[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.sync_sales_order_settlement_status(uuid[]) TO service_role;
...
BEGIN
  ...
  -- Keep the sale's own settlement flag in step: settled once no pending legs remain
  PERFORM public.sync_sales_order_settlement_status(v_settled_order_ids);
