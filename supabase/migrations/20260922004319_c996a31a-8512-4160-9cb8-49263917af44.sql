ALTER TABLE public.hr_hiring_requirements
  ADD COLUMN IF NOT EXISTS staffing_plan_id uuid REFERENCES public.hr_headcount_plans(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS source text NOT NULL DEFAULT 'manual';

ALTER TABLE public.hr_hiring_requirements
  DROP CONSTRAINT IF EXISTS hr_hiring_requirements_source_chk;
ALTER TABLE public.hr_hiring_requirements
  ADD CONSTRAINT hr_hiring_requirements_source_chk
  CHECK (source IN ('manual','capacity_occupancy','employee_separation'));

CREATE INDEX IF NOT EXISTS hr_hiring_req_plan_idx
  ON public.hr_hiring_requirements (staffing_plan_id, status);
CREATE UNIQUE INDEX IF NOT EXISTS hr_hiring_req_one_capacity_pending_uk
  ON public.hr_hiring_requirements (staffing_plan_id)
  WHERE source = 'capacity_occupancy' AND status = 'pending_approval' AND staffing_plan_id IS NOT NULL;
...
WITH current_shift AS (
  SELECT DISTINCT ON (sch.employee_id)
         sch.employee_id, sch.shift_id
...
reqs AS (
  SELECT hr.department_id, hr.position_id, hr.shift_id,
...
cand AS (
  SELECT r.department_id,
         ...
         hr.shift_id,
         count(*) FILTER (
           WHERE NOT coalesce(c.hired,false)
             AND NOT coalesce(c.canceled,false)
             AND NOT coalesce(c.converted,false)
         )::int AS pipeline_hc,
         count(*) FILTER (
           WHERE coalesce(c.hired,false)
             AND NOT coalesce(c.converted,false)
...
SELECT p.id,
       ...
       p.required_hc,
       CASE
         WHEN p.shift_id IS NULL THEN coalesce((
           SELECT max(ebs.current_hc)
           FROM emp_by_shift ebs
           WHERE ebs.department_id IS NOT DISTINCT FROM p.department_id
             AND ebs.position_id IS NOT DISTINCT FROM p.position_id
         ), 0)
         ELSE coalesce((
           SELECT ebs.current_hc
           FROM emp_by_shift ebs
           WHERE ebs.department_id IS NOT DISTINCT FROM p.department_id
             AND ebs.position_id IS NOT DISTINCT FROM p.position_id
             AND ebs.shift_id IS NOT DISTINCT FROM p.shift_id
         ), 0)
       END::int AS current_hc,
       coalesce(seats.physical_seats, 0)::int,
       coalesce(cand.pipeline_hc, 0)::int,
       coalesce(cand.selected_hc, 0)::int,
       coalesce(cand.pending_joining_hc, 0)::int,
       CASE
         WHEN p.shift_id IS NULL THEN coalesce((
           SELECT max(ebs.notice_hc)
           FROM emp_by_shift ebs
           WHERE ebs.department_id IS NOT DISTINCT FROM p.department_id
             AND ebs.position_id IS NOT DISTINCT FROM p.position_id
         ), 0)
         ELSE coalesce((
           SELECT ebs.notice_hc
           FROM emp_by_shift ebs
           WHERE ebs.department_id IS NOT DISTINCT FROM p.department_id
             AND ebs.position_id IS NOT DISTINCT FROM p.position_id
             AND ebs.shift_id IS NOT DISTINCT FROM p.shift_id
         ), 0)
       END::int AS notice_period_hc,
       p.target_date,
       p.priority,
...
LEFT JOIN reqs ON reqs.department_id IS NOT DISTINCT FROM p.department_id
              AND reqs.position_id IS NOT DISTINCT FROM p.position_id
              AND reqs.shift_id IS NOT DISTINCT FROM p.shift_id
LEFT JOIN cand ON cand.department_id IS NOT DISTINCT FROM p.department_id
              AND cand.position_id IS NOT DISTINCT FROM p.position_id
              AND cand.shift_id IS NOT DISTINCT FROM p.shift_id
LEFT JOIN seats ON seats.department_id IS NOT DISTINCT FROM p.department_id
               AND seats.position_id IS NOT DISTINCT FROM p.position_id
               AND seats.shift_id IS NOT DISTINCT FROM p.shift_id
               AND coalesce(seats.location, '-') = coalesce(p.location, '-')
WHERE p.is_active;
$$;
GRANT EXECUTE ON FUNCTION public.hr_workforce_staffing_matrix() TO authenticated;
GRANT EXECUTE ON FUNCTION public.hr_workforce_staffing_matrix() TO service_role;
...
GRANT EXECUTE ON FUNCTION public.hr_workforce_unplanned_scopes() TO service_role;
...
GRANT EXECUTE ON FUNCTION public.hr_reconcile_capacity_hiring(boolean) TO authenticated;

CREATE OR REPLACE FUNCTION public.hr_trigger_reconcile_capacity_hiring()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  END IF;
  PERFORM public.hr_reconcile_capacity_hiring(TG_TABLE_NAME = 'hr_seat_capacity');
  RETURN coalesce(NEW, OLD);
END;
$$;
...
AFTER INSERT OR UPDATE OF department_id, position_id, shift_id, location, physical_seats, max_operational_capacity, is_active
ON public.hr_seat_capacity
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();
...
AFTER INSERT OR UPDATE OF department_id, position_id, shift_id, location, approved_hc, required_hc, is_active
ON public.hr_headcount_plans
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();

DROP TRIGGER IF EXISTS trg_hr_work_info_reconcile ON public.hr_employee_work_info;
CREATE TRIGGER trg_hr_work_info_reconcile
AFTER INSERT OR UPDATE OF department_id, job_position_id, shift_id OR DELETE
ON public.hr_employee_work_info
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();

DROP TRIGGER IF EXISTS trg_hr_shift_schedule_reconcile ON public.hr_employee_shift_schedule;
CREATE TRIGGER trg_hr_shift_schedule_reconcile
AFTER INSERT OR UPDATE OF shift_id, effective_from, effective_to, is_current OR DELETE
ON public.hr_employee_shift_schedule
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();

DROP TRIGGER IF EXISTS trg_hr_employee_status_reconcile ON public.hr_employees;
CREATE TRIGGER trg_hr_employee_status_reconcile
AFTER UPDATE OF is_active, resignation_status
ON public.hr_employees
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();

DROP TRIGGER IF EXISTS zz_trg_hr_candidate_workforce_reconcile ON public.hr_candidates;
CREATE TRIGGER zz_trg_hr_candidate_workforce_reconcile
AFTER INSERT OR UPDATE OF recruitment_id, hired, canceled, converted, joining_date OR DELETE
ON public.hr_candidates
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();

DROP TRIGGER IF EXISTS trg_hr_requirement_workforce_reconcile ON public.hr_hiring_requirements;
CREATE TRIGGER trg_hr_requirement_workforce_reconcile
AFTER INSERT OR UPDATE OF number_required, positions_filled, status, staffing_plan_id OR DELETE
ON public.hr_hiring_requirements
FOR EACH STATEMENT EXECUTE FUNCTION public.hr_trigger_reconcile_capacity_hiring();